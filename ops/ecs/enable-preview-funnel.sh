...
controller_tailscale_funnel status --json > "$scratch/before.json"
...
if [[ "$before" == absent ]]; then
  controller_tailscale_funnel --https=443 --yes --bg http://127.0.0.1:8080
fi
controller_tailscale_funnel status --json > "$scratch/after.json"
...
if ! curl --fail --silent --show-error --proto '=https' --tlsv1.2 --max-time 20 "$origin/" -o "$body" || ! grep -Fq '<meta name="meetwise-preview-release"' "$body"; then
  if [[ "$before" == absent ]]; then controller_tailscale_funnel --https=443 off || true; fi
