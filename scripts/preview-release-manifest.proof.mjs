...
assert.throws(() => verifyManifest({ ...signed, origin: 'https://other.tail0000000.ts.net' }, publicPem, { now }), /signature_mismatch/);
...
assert.throws(() => signManifest({ ...valid, origin: 'http://preview.tail0000000.ts.net' }, privateKey.export({ type: 'pkcs8', format: 'pem' })), /origin_invalid/);
