...
async function transition(from, to) {
  await run(process.execPath, [ledger, 'transition', '--path', statePath, '--from', from, '--to', to, '--release', release, '--fingerprint', 'b'.repeat(64), '--origin', 'https://preview.tail0000000.ts.net', '--pages', 'disabled']);
}
...
try {
  ...
  assert.equal(assertFunnelAbsentOrPreview({ Web: {} }, 'preview.tail0000000.ts.net'), null);
  assert.equal(assertFunnelAbsentOrPreview({ Web: { 'preview.tail0000000.ts.net:443': { Handlers: { '/': { Proxy: 'http://127.0.0.1:8080' } } } } }, 'preview.tail0000000.ts.net'), 'https://preview.tail0000000.ts.net');
  assert.throws(() => assertFunnelAbsentOrPreview({ Web: { 'other.tail0000000.ts.net:443': { Handlers: { '/': { Proxy: 'http://127.0.0.1:8080' } } } } }, 'preview.tail0000000.ts.net'), /preview_funnel_/);
