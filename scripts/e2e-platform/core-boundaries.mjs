...
export function scanHelperSource(relPath, source) {
  ...
  if (APP_IMPORT.test(source)) errors.push(`e2e_core_boundaries_helper_imports_app:${relPath}`);
...
export function listHelperSources(repoRoot) {
  ...
  const files = readdirSync(helpersRoot)
    .filter((name) => (name.endsWith('.ts') || name.endsWith('.mjs')) && !name.endsWith('.e2e.ts'))
    .sort()
