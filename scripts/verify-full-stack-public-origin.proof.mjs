...
const origin = 'https://preview.tail39416d.ts.net';
...
await assert.rejects(() => verifyFullStackPublicOrigin({ origin, probeNonce: nonce, publicKeyPem: publicPem, fetchImpl: async (url, options) => url.endsWith('.json') ? response(url, JSON.stringify({ ...manifest, origin: 'https://evil.tail39416d.ts.net' })) : goodFetch(url, options), now }));
