...
test('flag 解析：只认 --core / --live / --dry-run，未知 flag 单独列出', () => {
  assert.deepEqual(parseRegressionArgs([]), {
    wantCore: false, wantLive: false, dryRun: false, claimFlags: [], unknown: [],
  });
  assert.deepEqual(parseRegressionArgs(['--core', '--live', '--dry-run']), {
    wantCore: true, wantLive: true, dryRun: true, claimFlags: [], unknown: [],
  });
  assert.deepEqual(parseRegressionArgs(['--help', '--core']), {
    wantCore: true, wantLive: false, dryRun: false, claimFlags: [], unknown: ['--help'],
  });
  assert.deepEqual(parseRegressionArgs(['--claim-done']), {
    wantCore: false, wantLive: false, dryRun: false, claimFlags: ['--claim-done'], unknown: [],
  });
