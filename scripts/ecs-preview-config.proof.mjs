...
const checks = [
  ...
  ['web maintains a matching public-preview depth gate without weakening normal auth',
    /resolvePublicPreview\(process\.env\.MEETWISE_PUBLIC_PREVIEW\)/.test(middleware)
      && /invalid_meetwise_public_preview/.test(middleware)
      ...
      && /NextResponse\.redirect\(authRedirectUrl\(request, pathname\)\)/.test(middleware)
      && /invalid_forwarded_host/.test(middleware)
      && /invalid_forwarded_proto/.test(middleware)
  ...
  ['a temporary Funnel is an explicit edge-probing phase and cannot be opened from loopback-only activation',
    /controller_ledger_transition active_unpublished edge_probing/.test(edgeProbe)
      ...
      && /systemctl reset-failed meetwise-preview-edge-probe-watchdog\.service meetwise-preview-edge-probe-expiry\.service \|\| true/.test(edgeProbe)
      && /controller_require_edge_probe_units_loaded/.test(edgeProbe)
      && /systemctl start meetwise-preview-edge-probe-watchdog\.service/.test(edgeProbe)
      && edgeProbe.indexOf('systemctl reset-failed') < edgeProbe.indexOf('controller_require_edge_probe_units_loaded')
      && edgeProbe.indexOf('controller_require_edge_probe_units_loaded') < edgeProbe.indexOf('controller_ledger_transition active_unpublished edge_probing')
      && edgeProbe.indexOf('controller_issue_serving_permit') < edgeProbe.indexOf('systemctl start meetwise-preview-edge-probe-watchdog.service')
      && edgeProbe.indexOf('systemctl start meetwise-preview-edge-probe-watchdog.service') < edgeProbe.indexOf('systemctl is-active --quiet meetwise-preview-edge-probe-watchdog.service')
