...
assert.match(root, /files\.sort\(\(a, b\) => Number\(a\) - Number\(b\)\)/, 'snapshot files must be ordered numerically once the controller closure exceeds ten rows');
const snapshotIndexes = Array.from({ length: 32 }, (_, index) => String(index));
assert.deepEqual([...snapshotIndexes].sort((a, b) => Number(a) - Number(b)), snapshotIndexes, '32-row snapshot indexes retain manifest order under numeric sorting');
assert.notDeepEqual([...snapshotIndexes].sort(), snapshotIndexes, 'lexical sorting would corrupt a 32-row snapshot and must not be used');
