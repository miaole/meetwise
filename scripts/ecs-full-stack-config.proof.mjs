...
check('Predecessor restore readback fails closed on systemd query errors', /active_state="\$\(timeout --kill-after=1s 5s systemctl show --property=ActiveState --value "\$unit" 2>\/dev\/null\)" \|\| die predecessor_owner_readback_failed 70/.test(cdRoot) && /\[\[ "\$active_state" =~ \^\(active\|inactive\|failed\)\$ \]\] \|\| die predecessor_owner_readback_failed 70/.test(cdRoot));
