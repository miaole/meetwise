...
check('Predecessor restore readback fails closed on systemd query errors', /active_state="\$\(timeout --kill-after=1s 5s systemctl show --property=ActiveState --value/.test(cdRoot) && /\[\[ "\$active_state" =~ \^\(active\|inactive\|failed\)\$ \]\]/.test(cdRoot) && /predecessor_owner_readback_failed/.test(cdRoot) && !/systemctl is-active "\$unit" 2>\/dev\/null \|\| true/.test(cdRoot));
