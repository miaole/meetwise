...
check('Predecessor snapshot uses one systemd read and treats only active transitions as owned', /'LoadState,ActiveState,UnitFileState,Restart'/.test(cdRoot) && /\['active', 'activating', 'reloading'\]\.includes\(observedActive\) \? 'active' : observedActive/.test(cdRoot) && !/observedActive === 'failed' && restart !== 'no'/.test(cdRoot));
check('Predecessor restore readback reuses the fail-closed systemd owner normalizer', (cdRoot.match(/read_legacy_units_json/g) ?? []).length === 3 && /legacy_units_json="\$\(read_legacy_units_json\)" \|\| die predecessor_legacy_state_invalid 70/.test(cdRoot) && /legacy_readback_json="\$\(read_legacy_units_json\)" \|\| die predecessor_owner_readback_failed 70/.test(cdRoot) && /s\.active==="active"/.test(cdRoot));
