...
check('API binds loopback both via an explicit service env and as the process default (fail-closed, not 0.0.0.0)', /Environment=HOST=127\.0\.0\.1/.test(text[files[0]]) && /process\.env\.HOST \|\| '127\.0\.0\.1'/.test(apiMain) && !/process\.env\.HOST \|\| '0\.0\.0\.0'/.test(apiMain));
