...
**本绿 ≠ 全链路 E2E covered**。`GAP-UC018-FULL-E2E`+#2 `GAP-UC018-GRAPH`+#3 `GAP-UC018-TTL`+#5 `GAP-UC018-UI`+#6 `GAP-UC018-SOLE`（PG-retained）已关；**#6 alone ≠ covered** → 矩阵仍 **partial** · **≠ covered**。
...
**仍 gap 于 covered（见 harness §1b）**：无单独 covered-lift authorize。UI（#5）+ sole PG-retained（#6 · `GAP-UC018-SOLE` CLOSED）已关 · **UI alone ≠ covered** · **#6 alone ≠ covered**。

**已关**：§1b#4 `waiting_user` CAS；**§1b#1 `GAP-UC018-FULL-E2E`**；**§1b#2 `GAP-UC018-GRAPH`**；**§1b#3 `GAP-UC018-TTL`**；**§1b#5 `GAP-UC018-UI`**（`pnpm uc018:ui:prove`）；**§1b#6 `GAP-UC018-SOLE`**（`pnpm uc018:sole:prove` · PG-retained · Ban MySQL/Qdrant sole-wiring）。矩阵仍 **partial** · **UI alone ≠ covered** · **#6 alone ≠ covered** · **≠ UC-E2E-018 covered**。
...
- [ ] 未把 `uc018:abandon:http:prove` / `uc018:abandon:full-e2e:prove` / `uc018:graph:prove` / `uc018:ttl:prove` / `uc018:ui:prove` / `uc018:sole:prove` / `commerce-reconcile:prove` 绿写成 UC-E2E-018 covered（FULL-E2E+GRAPH+TTL+UI+SOLE 已关仍 partial · UI alone ≠ covered · #6 alone ≠ covered）
...
2. §1b #1–#6 已关后，矩阵仍 **partial** · **#6 alone ≠ covered** — 是否另开 covered-lift authorize？（Ban claim covered this knife）  
