...
| UC-E2E-018 | **partial** | **partial** | **partial**（CAS waiting_user） | **blind** | FULL-E2E+GRAPH+TTL+UI+SOLE(PG-retained) 已关；**#6 alone ≠ covered** · **≠ covered** · matrix stays **partial** |
...
| UC-E2E-018 | 用户放弃面试 | **NON-UI** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + **`pnpm uc018:abandon:full-e2e:prove`** + **`pnpm uc018:graph:prove`** + **`pnpm uc018:ttl:prove`** + **`pnpm uc018:ui:prove`** + **`pnpm uc018:sole:prove`**（`GAP-UC018-FULL-E2E`+`GAP-UC018-GRAPH`+`GAP-UC018-TTL`+`GAP-UC018-UI`+`GAP-UC018-SOLE` **CLOSED** · sole=PG-retained）；harness `harness/uc-e2e-018-user-abandon.md` §1b + `eval/uc-e2e-018-user-abandon.eval.md`；旁证 `commerce:prove` / `commerce-reconcile:prove`（旁证≠TTL钉）/ `neg:interview` | **partial** | db+HTTP+**full.e2e**+**graph**+**ttl**+**UI abandon**+**sole PG-retained**；FULL-E2E+GRAPH+TTL+UI+SOLE 已关；**≠ covered**；**UI alone ≠ covered**；**#6 alone ≠ covered**；Ban wash MySQL/Qdrant sole-wiring；fixture=pgvector retained production-aligned（`adr-postgres-retained.md`）；本绿≠全链路 E2E；eval≠covered |
...
| P0-8 | UC-E2E-018 用户放弃 | **集成+HTTP+full.e2e+graph+ttl+UI+sole PG-retained prove 已挂** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + `pnpm uc018:graph:prove` + `pnpm uc018:ttl:prove` + `pnpm uc018:ui:prove` + `pnpm uc018:sole:prove` + harness §1b（FULL-E2E+GRAPH+TTL+UI+`GAP-UC018-SOLE` **CLOSED** under PG-retained）；**UI alone ≠ covered**；**#6 alone ≠ covered** | 矩阵 **partial**；本绿≠全链路 E2E covered；≠covered；Ban wash MySQL/Qdrant sole-wiring |
...
| UC-E2E-018 abandon | `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + `pnpm uc018:graph:prove` + `pnpm uc018:ttl:prove` + `pnpm uc018:ui:prove` | **0**/**0**/**0**/**0**/**0**/**0**（db+HTTP+full-e2e+graph+ttl+UI；R5） | db+HTTP+waiting_user+**FULL-E2E**+**GRAPH**+**TTL**+**UI**；**partial**；**UI alone ≠ covered**；≠covered；R5；仍缺 §1b #6 sole-stack |
