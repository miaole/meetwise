...
| UC-E2E-018 | **partial** | **partial** | **partial**（CAS waiting_user） | **blind** | FULL-E2E 已关；仍缺 TTL / UI / GRAPH / sole-stack · **≠ covered** |
...
| UC-E2E-018 | 用户放弃面试 | **NON-UI** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + **`pnpm uc018:abandon:full-e2e:prove`**（`e2e/full.e2e.ts` 显式 abandon TC；`GAP-UC018-FULL-E2E` **CLOSED**）；harness `harness/uc-e2e-018-user-abandon.md` §1b + `eval/uc-e2e-018-user-abandon.eval.md`；旁证 `commerce:prove` / `commerce-reconcile:prove` / `neg:interview` | **partial** | db+HTTP+**full.e2e abandon**→released+不可 resume；`GAP-UC018-FULL-E2E` 已关；**≠ covered**；仍缺 AiGraphRun safely_terminated / TTL 专用钉 / UI / sole-stack（§1b #2/#3/#5/#6；waiting_user+#1 已关）；fixture=pgvector → green-risk / R5；本绿≠全链路 E2E；eval≠covered |
...
| P0-8 | UC-E2E-018 用户放弃 | **集成+HTTP+full.e2e abandon prove 已挂** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + harness §1b（`GAP-UC018-FULL-E2E` **CLOSED**）；仍缺 AiGraphRun safely_terminated / TTL / UI / sole-stack（waiting_user+#1 已关） | 矩阵 **partial**；本绿≠全链路 E2E covered；≠covered |
...
| UC-E2E-018 abandon | `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` | **0**/**0**/**0**（2026-09-23 ~16:17 PDT；db+HTTP 46 PASS + full-e2e assertions=14；R5） | db+HTTP+waiting_user+**FULL-E2E**；**partial**；≠covered；R5；仍缺 §1b #2 GRAPH/#3 TTL/#5 UI/#6 sole-stack |
