...
| UC-E2E-050–052 | **partial**（**UC-052 deletion only**; 050/051 still **partial**/honesty-pin） | **partial**（**UC-052 deletion only**; 050/051 still **gap**） | **partial**（**UC-052 deletion only**; 050/051 still **blind**） | **blind** | Aggregate row · **only UC-052 deletion** NEG/FAULT/BOUND elevated（prove `3c4847a` · EOR `08d54f8`/`3e39c1e` · receipts supporting≠EOR）· **UC-050 kill-switch** / **UC-051 no-PII** / export **not** elevated（pre-nail §1.0.1 NEG cell=**partial**（NHP-050-NEG-01 **partial**/honesty-pin）· FAULT **gap** · BOUND **blind**）· public DELETE **503** · **≠ covered** · coveredCount **8** |
...
| UC-E2E-050–052 | kill-switch / 无 PII / 删除导出 | `privacy-erasure:http:prove`（DELETE=503 pin）；**`pnpm uc052:internal-erasure:prove`**（**UC-052 deletion** first knife · tip `3c4847a` · EOR `08d54f8`/`3e39c1e`）；harness `harness/uc-e2e-050-052-privacy-erasure.md` + `harness/privacy-erasure-http-503-pin.md` | **partial**（**UC-052 deletion only**）/ **blocked**（export · open DELETE） | **Only UC-052 deletion → partial**（locals PG purge · externals `retention_pending` · happy `pending_external` · FAULT-04 `purging`）· DELETE **仍必须 503**（GAP-PRIV-02 · Ban open）· Ban elevate UC-050 kill-switch / UC-051 PII / export · **本绿≠产品删除闭环** · **≠ covered** · checkpoint_rows fence-only · coveredCount **8** |
