...
| UC-E2E-018 | 用户放弃面试 | **NON-UI** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + **`pnpm uc018:abandon:full-e2e:prove`** + **`pnpm uc018:graph:prove`** + **`pnpm uc018:ttl:prove`** + **`pnpm uc018:ui:prove`** + **`pnpm uc018:sole:prove`** + **`pnpm uc018:covered-lift:prove`** + **`pnpm uc018:adv:prove`**（`GAP-UC018-FULL-E2E`+`GAP-UC018-GRAPH`+`GAP-UC018-TTL`+`GAP-UC018-UI`+`GAP-UC018-SOLE` **CLOSED** · sole=PG-retained · covered-lift **canHonestlyFlip=false** · refuse ADV was **blind** · tip `abfbbc0`；**ADV coding+prove** `harness/uc-e2e-018-adv.md` · **NHP-018-ADV-01** **partial** · `executed:awaiting_post_prove_dual` · **ADV alone ≠ covered** · Ban wash ADV into covered · Ban flip §1.1 covered · Ban skip to UC-011）；harness `harness/uc-e2e-018-user-abandon.md` §1b + `eval/uc-e2e-018-user-abandon.eval.md`；旁证 `commerce:prove` / `commerce-reconcile:prove`（旁证≠TTL钉）/ `neg:interview` | **partial** | db+HTTP+**full.e2e**+**graph**+**ttl**+**UI abandon**+**sole PG-retained**+covered-lift assessed+**ADV partial**；FULL-E2E+GRAPH+TTL+UI+SOLE 已关；§1.0 ADV **case-only→partial**；**canHonestlyFlip=false**（**ADV alone ≠ covered** · PERF/LOAD still blind）；**≠ covered** · **ADV alone ≠ covered**；**UI alone ≠ covered**；**#6 alone ≠ covered**；Ban假关；Ban wash SOLE/ADV alone into covered；Ban wash MySQL/Qdrant sole-wiring；fixture=pgvector retained production-aligned（`adr-postgres-retained.md`）；本绿≠全链路 E2E；eval≠covered；coveredCount **8** retained |
...
| P0-8 | UC-E2E-018 用户放弃 | **集成+HTTP+full.e2e+graph+ttl+UI+sole PG-retained+covered-lift+ADV prove 已挂** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + `pnpm uc018:graph:prove` + `pnpm uc018:ttl:prove` + `pnpm uc018:ui:prove` + `pnpm uc018:sole:prove` + `pnpm uc018:covered-lift:prove` + `pnpm uc018:adv:prove` + harness §1b（FULL-E2E+GRAPH+TTL+UI+`GAP-UC018-SOLE` **CLOSED** under PG-retained；covered-lift **canHonestlyFlip=false** · refuse ADV was **blind** · tip `abfbbc0`；**ADV** · **NHP-018-ADV-01** **partial** · `executed:awaiting_post_prove_dual` · **ADV alone ≠ covered**）；**UI alone ≠ covered**；**#6 alone ≠ covered**；Ban假关 | 矩阵 **partial**；§1.0 ADV **partial**；本绿≠全链路 E2E covered；≠covered；Ban wash SOLE/ADV alone into covered；Ban wash MySQL/Qdrant sole-wiring；coveredCount **8** retained |
...
| UC-E2E-018 abandon | `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + `pnpm uc018:graph:prove` + `pnpm uc018:ttl:prove` + `pnpm uc018:ui:prove` + `pnpm uc018:sole:prove` + `pnpm uc018:covered-lift:prove` + `pnpm uc018:adv:prove` | **0**×9（db+HTTP+full-e2e+graph+ttl+UI+sole+covered-lift+ADV；R5） | db+HTTP+waiting_user+**FULL-E2E**+**GRAPH**+**TTL**+**UI**+**SOLE**+covered-lift assessed+**ADV partial**；**partial**；**canHonestlyFlip=false**（**ADV alone ≠ covered** · PERF/LOAD still blind）；**UI alone ≠ covered** · **ADV alone ≠ covered**；**#6 alone ≠ covered**；≠covered；Ban假关；R5 |
