...
| UC-E2E-018 | **partial** | **partial** | **partial**（CAS waiting_user） | **blind** | FULL-E2E+GRAPH 已关；仍缺 TTL / UI / sole-stack · **≠ covered** |
...
| UC-E2E-018 | 用户放弃面试 | **NON-UI** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + **`pnpm uc018:abandon:full-e2e:prove`** + **`pnpm uc018:graph:prove`**（`GAP-UC018-FULL-E2E`+`GAP-UC018-GRAPH` **CLOSED**）；harness `harness/uc-e2e-018-user-abandon.md` §1b + `eval/uc-e2e-018-user-abandon.eval.md`；旁证 `commerce:prove` / `commerce-reconcile:prove` / `neg:interview` | **partial** | db+HTTP+**full.e2e abandon**+**graph safely_terminated**→released+不可 resume+图终态；FULL-E2E+GRAPH 已关；**≠ covered**；仍缺 TTL 专用钉 / UI / sole-stack（§1b #3/#5/#6；waiting_user+#1+#2 已关）；fixture=pgvector → green-risk / R5；本绿≠全链路 E2E；eval≠covered |
...
| P0-8 | UC-E2E-018 用户放弃 | **集成+HTTP+full.e2e+graph prove 已挂** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + `pnpm uc018:graph:prove` + harness §1b（FULL-E2E+GRAPH **CLOSED**）；仍缺 TTL / UI / sole-stack（waiting_user+#1+#2 已关） | 矩阵 **partial**；本绿≠全链路 E2E covered；≠covered |
...
| UC-E2E-018 abandon | `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + `pnpm uc018:graph:prove` | **0**/**0**/**0**/**0**（2026-09-23 ~16:43–16:44 PDT；db+HTTP 46 PASS + full-e2e assertions=14 + graph G1–G5；R5） | db+HTTP+waiting_user+**FULL-E2E**+**GRAPH**；**partial**；≠covered；R5；仍缺 §1b #3 TTL/#5 UI/#6 sole-stack |
