...
| UC-E2E-018 | **partial** | **partial** | **partial**（CAS waiting_user） | **blind** | 缺 full.e2e / TTL / UI |
...
| UC-E2E-018 | 用户放弃面试 | **NON-UI** `pnpm uc018:abandon:prove`（A1–A3+A-waiting-user db）+ `pnpm uc018:abandon:http:prove` → `apps/api/test/uc-e2e-018-user-abandon-http.proof.ts`（H1–H3+H-waiting-user 真 `POST /interview/:id/abandon`；CAS 含 waiting_user）；harness `harness/uc-e2e-018-user-abandon.md` §1b + `eval/uc-e2e-018-user-abandon.eval.md`；`commerce:prove` / `commerce-reconcile:prove` / `neg:interview` 旁证 | **partial** | db+HTTP abandon→released+不可复活+∉进行中；产品口已存在且 HTTP prove 已挂；**≠ covered**；仍缺 full.e2e 纳入 / AiGraphRun safely_terminated / TTL 专用钉 / UI / sole-stack（§1b；waiting_user CAS 已关）；fixture=pgvector → green-risk / R5；本绿≠全链路 E2E；eval≠covered |
...
| P0-8 | UC-E2E-018 用户放弃 | **集成+HTTP prove 已挂** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + harness §1b（A1–A3/H1–H3；真 `@Post(':id/abandon')`）；仍缺 full.e2e 纳入；AiGraphRun safely_terminated / TTL / UI / sole-stack（waiting_user CAS 已关） | 矩阵 **partial**；本绿≠全链路 E2E covered；≠covered |
...
| UC-E2E-018 abandon | `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` | **0**/**0**（2026-09-10 ~02:27 PT；db A1–A3+A-waiting-user + HTTP 46 PASS；R5） | db+HTTP+waiting_user CAS；**partial**；≠covered；无 Key；R5；仍缺 §1b full.e2e/AiGraphRun/TTL（waiting_user 已关） |
