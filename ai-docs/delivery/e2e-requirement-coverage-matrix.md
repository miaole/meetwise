...
| UC-E2E-018 | **partial** | **partial** | **partial**（CAS waiting_user） | **blind** | FULL-E2E+GRAPH+TTL+UI 已关；仍缺 sole-stack · **UI alone ≠ covered** · **≠ covered** |
...
| UC-E2E-018 | 用户放弃面试 | **NON-UI** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + **`pnpm uc018:abandon:full-e2e:prove`** + **`pnpm uc018:graph:prove`** + **`pnpm uc018:ttl:prove`** + **`pnpm uc018:ui:prove`**（`GAP-UC018-FULL-E2E`+`GAP-UC018-GRAPH`+`GAP-UC018-TTL`+`GAP-UC018-UI` **CLOSED**）；harness `harness/uc-e2e-018-user-abandon.md` §1b + `eval/uc-e2e-018-user-abandon.eval.md`；旁证 `commerce:prove` / `commerce-reconcile:prove`（旁证≠TTL钉）/ `neg:interview` | **partial** | db+HTTP+**full.e2e**+**graph**+**ttl**+**UI abandon**→released+不可 resume；FULL-E2E+GRAPH+TTL+UI 已关；**≠ covered**；**UI alone ≠ covered**；仍缺 sole-stack（§1b #6）；fixture=pgvector → green-risk / R5；本绿≠全链路 E2E；eval≠covered |
...
| P0-8 | UC-E2E-018 用户放弃 | **集成+HTTP+full.e2e+graph+ttl+UI prove 已挂** `pnpm uc018:abandon:prove` + `pnpm uc018:abandon:http:prove` + `pnpm uc018:abandon:full-e2e:prove` + `pnpm uc018:graph:prove` + `pnpm uc018:ttl:prove` + `pnpm uc018:ui:prove` + harness §1b（FULL-E2E+GRAPH+TTL+UI **CLOSED**）；仍缺 sole-stack（#6）；**UI alone ≠ covered** | 矩阵 **partial**；本绿≠全链路 E2E covered；≠covered |
