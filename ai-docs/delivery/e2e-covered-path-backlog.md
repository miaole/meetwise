...
| 018 | HTTP prove 已挂（真 `POST /interview/:id/abandon`）；仍缺 full.e2e 纳入；AiGraphRun safely_terminated；TTL；waiting_user；UI；sole-stack（harness §1b） |
...
1. UC-018 full.e2e 纳入 + AiGraphRun safely_terminated / waiting_user（HTTP prove 已挂；仍 ≠ covered）
