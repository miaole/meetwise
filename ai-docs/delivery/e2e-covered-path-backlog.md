...
| 018 | HTTP + **full.e2e abandon** prove 已挂（`GAP-UC018-FULL-E2E` CLOSED）；仍缺 AiGraphRun safely_terminated；TTL；UI；sole-stack（waiting_user+#1 已关；harness §1b）· 仍 **partial≠covered** |
...
1. UC-018 AiGraphRun safely_terminated / TTL（full.e2e abandon + waiting_user 已关；仍 ≠ covered）
