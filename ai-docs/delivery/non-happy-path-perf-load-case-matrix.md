...
| NHP-018-PERF-01 | 018 | PERF | api | `POST /api/interview/:id/abandon` 延迟（隔离 · N=100 · concurrency=10 · ≤2vCPU/4GiB） | p50≤250ms · p95≤750ms · p99≤1500ms · error_rate≤0.5% · 收据 `.tmp/uc018-perf-load-receipts/nhp-018-perf-01.json`；**≠** 线上 SLO · **≠** 容量 · **≠** HA | **blind**→**case-only** | REQUEST OPEN `harness/uc-e2e-018-perf-load.md` · Ban elevate to **partial** this open · Ban invent prove green · Ban post-hoc retune · Ban n/a dodge · PERF alone ≠ covered · cite `testing/e2e-performance-evidence.md` · matrix §0.5/§1.0 |
| NHP-018-LOAD-01 | 018 | LOAD | worker/api | 并发 abandon + release + graph safe-terminate（N=50 · concurrency=20） | 无双放 · 无 stuck reservations · error_rate≤1% · 吞吐入收据 `.tmp/uc018-perf-load-receipts/nhp-018-load-01.json`；**≠** HA · **≠** 生产容量 | **blind**→**case-only** | REQUEST OPEN `harness/uc-e2e-018-perf-load.md` · Ban elevate to **partial** this open · Ban invent prove green · Ban post-hoc retune · LOAD alone ≠ covered · cite reassess/`0b7a218` refuse PERF/LOAD blind |
