...
verification: pending_always_on
...
`pnpm regression` 叠到 `origin/main` @ `eb3f6ef`（#65+#68+#69+#71+#70+#66+#73+#74+#75+#77+#82）后的 always-on 回执待本轮跑完再填。`docs:check` 现为 72 个 required files（+ `privacy-erasure-preview-path.md`）。这只证明 always-on + 已接线可选静态门（含 `interview-dispatch:unit:prove`、`privacy-erasure-preview:domain:prove`、`privacy-erasure-preview:contract:prove`），不是 CI `verify`、不是 `--core`、不是 live E2E。隔离 `ocr:prove` / `int-answer-dual-write-fence:prove` / `interview-dispatch:prove` / `privacy-erasure-preview:prove` 需远程/CI Postgres，未跑、不进 always-on。本 PR 只 supersede #55–#64，不 supersede #82 / #77 / #75 / #74 / #73 / #66 / #70。作者不得自签 `review: passed`。不得 `--claim-done`。不得写「本轮局部验证完成」——审核仍是 `blocked:author_only`。
