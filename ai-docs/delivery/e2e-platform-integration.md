...
- `provider-egress-inventory`：同时登记 `run-post-change-regression.mjs` 与 `.proof.mjs`；本集成再登记 #62/#63 的 `e2e-fake-service-flags.mjs` / `e2e-static-guards*.mjs` / `review-loop.mjs`，以及 #67 `native-fail-closed.proof.ts`。`environmentReferenceCount` 随清单长度更新（当前 **222**），禁止手改成旧数。
...
可选（`package.json` 有脚本才挂）：`public-text-policy:prove` · `quality:traceability:prove` · `provider-egress:prove` · `public-preview-write:prove` · `public-preview-write-gate:prove` · `interview-answer-submission:prove` · `adaptive-length:prove` · `scor-00-honesty:prove` · `model-op01:prove` · `interview-dispatch:unit:prove` · `interview-dispatch:gate:prove` · `last-event-id:unit:prove` · `privacy-erasure-preview:domain:prove` · `privacy-erasure-preview:contract:prove` · `adaptive-signals:prove` · `adaptive-signals-graph:prove` · `signal-sse:prove` · `signal-sse-contract:prove` · `signal-sse-worker:prove` · `interview-voice-seams:prove` · `voice-stream-asr-honesty:prove` · `model-slot-bypass:static:prove` · `quiz-dual-claim:unit:prove` · `owner-drain-order:unit:prove` · `sse-slot:prove` · `native-fail-closed:prove` · `question-generation-fail-closed:prove`。隔离 `interview-dispatch:prove` / `model-slot-bypass:prove` / `quiz-dual-claim:prove` / `runtime:claim-join:prove` / `quiz:prove` / `reaper:prove` **不进** always-on（#97 把后两门挂进 per-push `verify`，不是 always-on）。

## 相对最新 main 的叠底（#65 + #68 + #69 + #71 + #70 + #66 + #73 + #74 + #75 + #77 + #82 + #72 + #79 + #83 + #67 + #80 + #84 + #78 + #88 + #86 + #87 + #81 + #90 + #89 + #95 + #91 + #93 + #92 + #94 + #98 + #96 + #97）

本分支已叠到 `origin/main` @ `e4e0d58`（#68 → #65 → #69 → #71 → #70 `0125` → #66 覆盖驱动软预算 → #73 `SCOR-00H` → #74 `0126` 答题双写互斥 → #75 `0127` 预览 OCR binding → #77 `0128` 面试公平调度 → #82 `0129` 隐私删除预览 → #72 `0130` SIGNAL-01 → #79 预览批量 ASR/TTS → #83 预览 `/resume` 图片入口 → #67 出题/原生 fail-closed → #80 预览 `POST /interview/:id/answers` → #84 SIGNAL-SSE `session_concluded` → #78 Pages 预览展示 → #88 Pages 仅面试练习证明 → #86 预览路径诚实闸 → #87 高并发复核骨架 + `parseLastEventId` → #81 招聘方内部预览笔记 → #90 `HC-GAP-014` 前端 400 停转 → #89 `HC-GAP-006` 押题/诊断 HTTP 400 → #95 流式 ASR 诚实双旗 → #91 `HC-GAP-009` 静态门 → #93 `HC-GAP-004` 押题/诊断双连接恰一领 → #92 `HC-GAP-011` 具名 claim-join 孤儿 → #94 `HC-GAP-002` 押题/诊断/报告按 owner 抽干 → #98 `HC-GAP-007` 共享 SSE 槽 5+1 → 429 → #96 `HC-GAP-001` 门 + 回执路径 → #97 `HC-GAP-012` 四门 130 迁移回执）。`package.json` / `run-e2e-isolated.mjs` 自动并集（保留 `tagE2EFailure` / `assertNoFakeServiceFlags` / `VOICE_FAKE`+`ASR_FAKE`+`TTS_FAKE`+`OCR_FAKE` **和** `runtime:claim-join:prove:raw`）；`interview-dispatch:prove` 走 #96 包装器，**不**改走 `run-e2e-isolated`；`ci.yml` 并入 #97 per-push `quiz:prove` / `reaper:prove`（**不进** always-on）；`check-docs.mjs` / `current-runtime-truth.md` 按并集解。迁移计数仍为经 `0130` 共 130（#97 / #96 / #98 / #94 / #92 / #93 / #91 / #95 / #89 / #90 / #81 / #87 / #86 / #88 / #78 / #84 / #80 / #67 / #83 / #79 无新迁移）。#97 / #96 / #98 / #94 / #92 / #93 / #91 / #95 / #89 / #90 / #81 / #87 / #86 / #88 / #78 / #84 / #80 / #67 / #83 / #79 / #72 / #82 / #77 / #75 / #74 / #73 / #66 / #70 是 main 上已合并的独立切片，**不是**本 PR 的 supersede 对象；本 PR 只 supersede #55–#64。
...
| #69 | 公开 `DELETE /privacy/interview-data/:id` 仍 `503 interview_erasure_authorization_not_available`；**生产**无公开 `/answers`（非预览 404）；`interview-answer-submission:prove` 冻结 submission/receipt 合同且不进 OpenAPI；签发器/0091 账本 ≠ 删除已开放；预览 `/turn` 503 不是隐私 DELETE 503，也不关闭 `INT-P0-RAW-QUEUE` |
...
| #77 | `0128_interview_dispatch_fairness.sql` 面试公平调度：`gateway_dispatch_owners` 按最老等待 owner 排序；tick 量子轮转 `fairDrainInterviewOwners`，不再抽干单 owner；每 owner 未过期 `running` cap 默认 1；进程内 `WORKER_INTERVIEW_GLOBAL_INFLIGHT` 默认 4 **不是**集群锁；`idle`=claim null，隐私归还/丢租约/graph fence/`markDone` CAS=0 为 `retry`。押题/诊断/报告仍抽干。可选 always-on 挂静态 `interview-dispatch:unit:prove`。#96 另关 `HC-GAP-001` 门 + 回执路径。不是延迟/容量 SLO |
| #96 | `HC-GAP-001`：`pnpm interview-dispatch:prove` 经 `scripts/run-interview-dispatch-prove.mjs` 缺远程配置失败关闭（`interview_dispatch_prove_requires_remote_postgres`），禁止 isolated profile / loopback / compose / `DATABASE_URL` / `run-e2e-isolated`。可选 always-on 挂无库 `pnpm interview-dispatch:gate:prove`（已在 per-push CI）。隔离 `pnpm interview-dispatch:prove` **不进** always-on；本环境无远程通过回执。关闭的是门 + 回执路径，不是远程 SQL 绿。无新迁移 |
| #97 | `HC-GAP-012`：四门 130 迁移回执钉在 CI run `33867570523` / tip `06b46c4`（commerce 50/50、report 31/31、quiz 22/22、reaper 28/28）。per-push `verify` 挂 `quiz:prove` / `reaper:prove`。**不进** always-on。不是支付渠道、云故障或发布证据。无新迁移 |
...
| #72 | `0130_model_invocation_same_key_claim_join.sql` + SIGNAL-01 控制流 hook：`observeInterviewSignals` + `decideNext` 可结论 `early_weak` / `thrashing`；图 `decide` 把 `concludeReason` 写成 `DecisionProvenance`。报告路径仍不读。#84 另把 `early_weak`/`thrashing` 预览投影到既有 SSE `session_concluded`（不是终态）。不是 ScoreCard、B 端 band、`INT-LEVEL-01`，也不是产品硬顶 8 轮。可选 always-on 挂静态 `adaptive-signals:prove` / `adaptive-signals-graph:prove`。不把 main 较弱的 runner（无 `tagE2EFailure` / 无假服务列表 / 弱 Key trim）覆盖本集成 |
...
| #80 | 预览答题账本：`MEETWISE_PUBLIC_PREVIEW=1` 下 `POST /interview/:id/answers` → 既有 `0092` `submitInterviewAnswer`；受 `0126` 围栏；非预览 404；语音 `/transcribe` `/speak` 仍 503。无新迁移（不占 0131）。**不是** `INT-TRANSCRIPT-01`，不重开生产删除。隔离 `int-transcript-preview-submit:http:prove` 需远程 Postgres，**不进** always-on |
| #84 | SIGNAL-SSE 预览投影：`early_weak`/`thrashing` → 既有 `interview_event` / SSE `session_concluded`（`event_key=session_concluded`）；载荷无 score/overall/band；其他 conclude 码不写。`session_concluded` **不是**终态。无新迁移。不是 `INT-LEVEL-01` / ScoreCard。可选 always-on 挂静态 `signal-sse:prove` / `signal-sse-contract:prove` / `signal-sse-worker:prove` |
| #78 | Pages 预览展示：仓库 `docs/index.html` + `.github/workflows/pages.yml` 复制 `docs/` 与合成截图。不是应用运行时 / 数据面 / 发布证据。公开文案只谈面试练习，招聘不在预览范围。无新迁移 |
| #88 | Pages 证明收紧：`docs:check` 锁定 `pages.yml` 与「仅面试练习 / 招聘不在本预览范围」；runtime-truth 断言 `docs/index.html` → `仅面试练习`。不是应用运行时 / 发布证据。无新迁移 |
| #86 | 预览路径诚实闸：inventory 覆盖 async `transcribe`/`speak`/`speakStream`；`preview-controlled-write` + `assertPublicPreviewControlledWriteAllowed` / `PREVIEW_CONTROLLED_WRITE`；公开预览下语音三路由仍 503，只额外放行 `POST /interview/:id/answers`。不含 `MEETWISE_PUBLIC_PREVIEW=1` 且能力 Key 存在时，预览批处理语音可走 `/transcribe`+`/speak`。检索未命中可走 CRAG Web + LLM，不是 canned-stem；供应商/native miss 仍 `generation_unavailable`。git 主线迁移事实是 `0130`（130 个），不得再写「远端 main 只有 0120」。Pages 文案不得写成「预览不接受作答」。无新迁移 |
| #87 | 高并发复核骨架：`high-concurrency-review.md` 记录五面接线与 `HC-GAP-001`…`014`；不是容量/集群锁/延迟 SLO。`parseLastEventId` 把押题/诊断/面试非法 `Last-Event-ID` 失败关闭，不得降级为 `0` 全量重放。可选 always-on 挂静态 `last-event-id:unit:prove`。押题/诊断 HTTP 400 现由 #89 `api:validate` 关闭。无新迁移 |
| #81 | 招聘方内部预览：`bend-recruiter-architecture-surface.md` + `/recruiter/how-it-works` 架构笔记与只读申请状态。不是「求职者 / 面试官」两套对等产品，招聘方产品没有上线。不是高峰容量保证。无新迁移 |
| #90 | `HC-GAP-014`：面试 / 押题 / 诊断 SSE 驱动把 HTTP 400 `invalid_last_event_id` 当失败关闭，不是断线；不得用同一游标重试。本地证明 `pnpm web:prove`。无新迁移 |
| #89 | `HC-GAP-006`：`pnpm api:validate` 对面试 / 押题 / 诊断非法 `Last-Event-ID` 断言 HTTP 400（同一组坏游标；`Infinity` 不触发 catch-up SQL）。不证明 429 槽打满（`HC-GAP-007`）或跨副本槽（`HC-GAP-008`）。无新迁移 |
| #95 | 流式 ASR / turn-taking 诚实闸：精确双旗 `VOICE_STREAM_ASR_ENABLED=1` **且** `VOICE_STREAM_ASR_PREVIEW=1`；生产 / enforce / 公开预览仍拒；Key 单独不能开 WebSocket；组合根不接线 live stream。`pnpm voice-stream-asr-honesty:prove`。不是产品流式 E2E，不关闭 `PRD-TEST-006`。无新迁移 |
| #91 | `HC-GAP-009`：legacy invoke 无 `operation` 不进 0120 租约；第三条有 operation 槽拒绝。可选 always-on 挂无库 `pnpm model-slot-bypass:static:prove`（已在 per-push CI）。隔离 `pnpm model-slot-bypass:prove` 需 Docker，**不进** always-on，本环境无回执。不 fail-close 旧缝，不关闭 `HC-GAP-010`。无新迁移 |
...
| `architecture/ai/provider-egress-inventory.{json,md}` | 登记 #62/#63 E2E 引用 + #79 `interview-voice-seams` + #67 `native-fail-closed` + #95 `voice-stream-preview`；`environmentReferenceCount` = 222；consumer pairs = 33 |

`docs:check` 把本文件列为 required，并要求出现 `#55`、`#64`、`#69`、`#71`、`#70`、`#66`、`#73`、`#74`、`#75`、`#77`、`#82`、`#72`、`#79`、`#83`、`#67`、`#80`、`#84`、`#78`、`#88`、`#86`、`#87`、`#81`、`#90`、`#89`、`#95`、`#91`、`#93`、`#92`、`#94`、`#98`、`#96`、`#97`、`feature/e2e-platform-integration`、`fail-closed`、`releaseEvidence`、`supersede`。
...
叠到 `origin/main` @ `e4e0d58`（+#97）后仅做冲突并集，本 tip 未重跑 `pnpm regression`。`docs:check` 现为 80 个 required files。迁移计数仍为 130。本 PR 只 supersede #55–#64，不 supersede #97 / #96 / #98 / #94 / #92 / #93 / #91 / #95 / #89 / #90 / #81 / #87 / #86 / #88 / #78 / #84 / #80 / #67 / #83 / #79 / #72 / #82 / #77 / #75 / #74 / #73 / #66 / #70。作者不得自签 `review: passed`。不得 `--claim-done`。不得写「本轮局部验证完成」——审核仍是 `blocked:author_only`。
