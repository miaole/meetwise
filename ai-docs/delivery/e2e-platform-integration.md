...
叠到 `origin/main` @ `76d5d0f`（#65+#68+#69+#71+#70+#66+#73+#74+#75）后，`pnpm regression` 尚未重跑。本轮诚实边界待 always-on 退出后再改 `verification`。`ocr:prove` 与 `int-answer-dual-write-fence:prove` 需远程/CI Postgres，未跑、不进 always-on。这不是 CI `verify`、不是 `--core`、不是 live E2E。本 PR 只 supersede #55–#64，不 supersede #75 / #74 / #73 / #66 / #70。作者不得自签 `review: passed`。不得 `--claim-done`。不得写「本轮局部验证完成」——审核仍是 `blocked:author_only`。
