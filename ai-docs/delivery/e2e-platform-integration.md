...
`pnpm regression` 在叠到 `origin/main` @ `88c9a71`（#65+#68+#69+#71+#70+#66+#73）后退出 0（`outcome=passed_always_on`）。`docs:check` 现为 67 个 required files。这只证明 always-on + 已接线可选静态门（含 `adaptive-length:prove` 与 `scor-00-honesty:prove`），不是 CI `verify`、不是 `--core`、不是 live E2E。本 PR 只 supersede #55–#64，不 supersede #73 / #66 / #70。作者不得自签 `review: passed`。不得 `--claim-done`。不得写「本轮局部验证完成」——审核仍是 `blocked:author_only`。
