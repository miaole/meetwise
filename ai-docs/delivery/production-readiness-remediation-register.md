...
> 当前自适应面试保持“当前题答 + 覆盖/证据政策收口 + 安全天花板 16”的有界模型，不把它改造成无界聊天。固定 8 轮硬顶已废除。下表仅在产品确实需要长自由对话时执行；每个子项均须单独完成、验证和关闭，不能只做模型摘要就宣称完成。
