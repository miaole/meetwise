...
- [ ] `PUBLIC-PREVIEW-WRITE-GATE-01`：公开预览 API 仅 allowlist `GET`、`HEAD`、`OPTIONS`，其余所有方法在 Fastify `onRequest` 前置门固定 `503 public_preview_read_only`。该项的 `TC-public-preview-01-main/E1…E6` 是 planned/unmapped；在真实 Fastify ingress proof、ECS listener 模式/镜像摘要/健康回执进入同一发布清单前，Pages 的主项目入口保持禁用，ECS 不开放公开主应用。
