...
- [◐] `INT-TRANSCRIPT-00`：独立 `PrivacyAuthorizationIssuer`（ECDSA P-256 / ES256，`iss=meetwise-privacy-authz-v1`）与 0091 `privacy_authorization_snapshot` / `privacy_deletion_receipt`、受约束 claim、no-forge-completed guard 已在源码落地；signer/verifier 与 `AUTH_SECRET`、runtime SQL、worker/deleter、GUC 身份根分离。`0091` issue 按调用方字段落账，本身不做 JWS 验签；privacy worker 仍走 `0077`，HTTP 未接线。submission/receipt 合同已冻结且不进 OpenAPI。公开 `DELETE /privacy/interview-data/:id` 仍 503。`0129` 预览版 `POST /privacy/erasure-preview` 可盘点 sink 并链接 0096/0125 begin，回执固定未完成、`releaseEvidence=false`，不是 issuer 生产删除。树上已有 0092 rehearsal 表/函数与 0096 event/report/`ai_graph_run` rehearsal resolver。预览版可将 HTTP `POST /interview/:id/answers` 接到 `submitInterviewAnswer`（`INT-TRANSCRIPT-PREVIEW-SUBMIT`）；非预览该路径 404，OpenAPI 不登记。这**不**授权 `INT-TRANSCRIPT-01` 生产 cutover，也不把 rehearsal purge 或预览删除回执称为公开删除已闭环。七类 TC 仍 planned/unmapped；账本 HTTP 证明须远程 Postgres 环境变量，禁止 `pnpm db:up`，无回执时 `releaseEvidence=false`。`pnpm mem00-int00:prove-path` 编排可移植 pin + 隔离入口；本环境 Docker 缺失时隔离项记 blocked，**不**伪称控制面已关。无 receipt、无组合根滥用证明前不得声称删除完成，也不得勾“已关闭”。现有 legacy `/turn` 仍写 plaintext job payload，必须如实保留为 `INT-P0-RAW-QUEUE`，不可被文字误称为已停用。`PUBLIC-PREVIEW-WRITE-GATE-01` 在预览下仍拒绝 `/turn`，并允许上述受控账本写；`MEETWISE_PUBLIC_PREVIEW=1` 下 `POST /privacy/erasure-preview` 仍 503。
...
| 当前状态 | ◐；`0093`/`memory-governance` 与后续 MEM 迁移已在源码落地，`pnpm memory-governance:prove` / `memory-control-surface:prove` / `memory:prove` 入口已接线。本环境 Docker 缺失时隔离证明为 `blocked:docker_daemon_missing`，可移植 pin（`privacy-authorization:crypto`、erasure-preview domain/contract、memory-vector-chunk domain）可绿。编排入口 `pnpm mem00-int00:prove-path`（`releaseEvidence=false`）。现有 lean memory 仍只能做精确题目去重和 report 弱项软排序，**不能**称为长期语义记忆或控制面已关；公开删除仍 503，不得勾已关闭。 |
| 目标 | 先形成控制面的授权、生命周期、删除、重建、审计和跨租户隔离，再允许写入长期对话、事实、摘要或向量。 |
| 依赖 | 无；是 `EXEC-06` 的硬前置。 |

执行清单：

- [◐] `MEM-00`：**已发现/已实现（源码）/未验证（本环境隔离库）**。迁移 `0093` + `packages/db` memory-governance 与复用 `PrivacyAuthorizationIssuer` 账户删除链已在树；证明入口 `pnpm memory-governance:prove`（隔离 PG）+ 可移植域 pin。本机 Docker 不可用 → 隔离证明 blocked，不得填通过回执、不得勾已关闭。编排见 `pnpm mem00-int00:prove-path`（`releaseEvidence=false`）。不可复用可写路由 GUC 作为身份根；长期写入仍禁止。
- [◐] `MEM-10`：**已发现/已实现（源码 `0107`）/未验证（本环境隔离库）**。`pnpm memory-control-surface:prove` 入口已接线；Docker 缺失时 blocked。不关闭 MEM-00，不开放生产长期写入。`releaseEvidence=false`。
