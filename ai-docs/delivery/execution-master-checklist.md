...
- [ ] 在 `INT-LONG-INTERVIEW-01`、`INT-LEVEL-01` 和 `SCOR-01…07` 完成前，不把现有八轮自适应流程称为一到两小时专家面试，也不按工作年限、单题高分或未校准 overall score 给出等级、排序或招聘结论。
...
| 1 | `EXEC-01A` 长时专家面试会话与 Graph 安全基础 | ☐ | 无；涉及模型派发的子项还依赖 `EXEC-02` | 完整可恢复 transcript、冻结面试蓝图、动态能力校准和安全围栏都有独立业务真相，且不夸大为已上线。 |
...
- [ ] `PUBLIC-PREVIEW-DIRECTORY-01`：Pages 只发布 `preview-site/`，静态目录没有 API、IP、端口、数据服务、秘密或运行时跳转输入；默认分支与最低 Pages 权限是唯一发布来源。`TC-public-preview-directory-01-main/E1…E6` 为 planned/unmapped，静态 proof 不是部署或 ECS 证据。
- [◐] `PUBLIC-PREVIEW-WRITE-GATE-01`：公开预览 API 仅 allowlist `GET`、`HEAD`、`OPTIONS`，其余所有方法在 NestJS(Fastify) `onRequest` 前置门固定 `503 public_preview_read_only`。面试/评分写面另有服务层 `assertPublicPreviewWritesClosed`（`InterviewService` 写方法与 `ApplicationsService.start`/`finalize`）和版本化清单 `ai-docs/architecture/backend/public-preview-write-inventory.json`。本地命令 `pnpm public-preview-write:inventory`、`pnpm public-preview-write:prove`、`pnpm public-preview-write-gate:prove`、`pnpm -C apps/web prove:middleware` 已接线，回执必须保持 `releaseEvidence=false`。该项的 `TC-public-preview-01-main/E1…E6` 仍是 planned/unmapped；在 ECS listener 模式/镜像摘要/健康回执进入同一发布清单前，主项目入口保持禁用，ECS 不开放公开主应用。不得把本地清单或 inject proof 标成已关闭，也不等于预览 worker 已停。
...
- [ ] `WORKER-DISPATCH-002`：在 `WORKER-DISPATCH-001` 后处理按 owner 全量串行 drain 的 head-of-line 阻塞；冻结公平轮转、每 owner/global 并发和模型 operation 预算，保持同一面试严格保序。该项未完成前，不把即时 wakeup 宣称为繁忙状态的端到端延迟或容量保证。
...
| 字段 | 内容 |
| --- | --- |
| 对应登记 | `INT-TRANSCRIPT-00`、`INT-TRANSCRIPT-01`、`INT-RESUME-02`、`INT-LEVEL-01`、`INT-LONG-INTERVIEW-01`、`SEC-GRAPH-01`；完整用例见 `requirements/use-cases/expert-long-interview-runtime.md`。 |
| 当前状态 | blocked；2026-08-13 的独立数据一致性与 AI/运行时审查确认删除授权、raw answer 落点和 response-lost 回执均为 P0。当前自适应图以有界当前题答为主，默认/硬上限为八轮；checkpoint 只足以恢复 pending graph 工作，raw answer 在 answer job 终态前仍为明文 payload，SSE/client 状态不构成历史面试账本。 |
...
| 依赖 | `INT-TRANSCRIPT-00` 的独立 `PrivacyAuthorizationIssuer`（不得复用 `AUTH_SECRET`、runtime SQL、worker/deleter 或 GUC 身份根）、不可伪造删除授权、target/sink receipt 与 submission/receipt **合同冻结**是 01 的 P0 前置；00 本身不授权写入、也不产生 artifact sink。只有 01 的新 schema、target resolver、deletion ledger、逐 sink receipt 和删后 read=0 在同一迁移、同一真实组合根证明后，真实用户 write route 才能启用；此前一律 disabled。评分合同依赖 `EXEC-01`；模型外送/压缩/分类的 operation 依赖 `EXEC-02`；长期跨会话记忆依赖 `EXEC-03` 与 `EXEC-06`；题库 scope 依赖 `EXEC-04`。01 首包禁止模型、RAG/Web、评分、报告和 memory 副作用。 |

执行清单：

- [ ] `INT-TRANSCRIPT-00`：先实施独立 `PrivacyAuthorizationIssuer`，签发不可伪造、短时、单次且 object/owner/purpose/epoch/target-set digest 绑定的删除授权快照；其 signer/verifier、`issuerId/keyId/jti` 消费账本不得复用 `AUTH_SECRET`、runtime SQL、worker/deleter 或 GUC 身份根。再实施受约束 deleter、target ledger 与逐 sink receipt；本包只冻结 submission/receipt 合同，**不**创建 artifact/draft/item/new-sink，也不授权任何新的 01 canonical raw write。公开删除入口保持 503，直至 forged GUC/登录令牌/raw SQL/cross-owner/issuer/key/jti、delete×submit/worker/provider late-result 和恢复用例在真实组合根通过；无 receipt 不得声称删除完成。现有 legacy `/turn` 仍写 plaintext job payload，必须如实保留为 `INT-P0-RAW-QUEUE`，不可被文字误称为已停用。
...
- [ ] `INT-LONG-INTERVIEW-01`：以不可变 `InterviewBlueprintSnapshot` 固定时长、模块、能力覆盖、route allocation、最大题数、每模块最少有效证据、版本和终止策略；终止从“固定轮数”改为 time + coverage + evidence 的确定性判定。当前 8 轮只保留为现有产品边界，不可把数字调大当作完成。
...
| 当前状态 | `MODEL-OP-00` 为 **P0 blocked**：文本路径已有冻结 `max_tokens`、局部预算与 price revision 绑定。工作树候选 `0088` 已撤销 runtime role 对 invocation 的 INSERT/UPDATE/DELETE，并以固定过程加 permit trigger 替换 `0085` 的单一更新围栏；`pnpm model-op00:prove` 已于 2026-08-16 在隔离 PostgreSQL 跑绿（89 迁移、exit 0、`releaseEvidence=false`），direct INSERT/非法 terminal/identity mutation/reservation mismatch 被真实低权 SQL 拒绝；但尚未独立复审，且 catalog 未在主链、node identity 仍可被 caller 参与，故 raw-SQL 状态机安全仅有本地证据、`MODEL-OP-00` 整体仍未关闭。`MODEL-OP-01` 同样被未接线 registry、caller-controlled node identity、broad DashScope native secret 与 OCR/voice/embed direct adapter 阻断。原生 URL 已改为固定 Beijing endpoint profile，旧 URL 环境变量和 production/development override 均 fail-closed；这是 `BAILIAN-04` 的静态局部实现，不解除 `MODEL-OP-01`。无策略文本的 production/enforce fence 与 OCR 的全环境 fail-closed 开关也只是止血，不是 operation 级治理。所有回执均为 `releaseEvidence=false`。 |
...
- [ ] `BAILIAN-06`：视觉、embedding、ASR、TTS 和流式能力逐项拥有数据上限、取消、费用与隐私 smoke；未获准项维持关闭。
...
| 当前状态 | ◐，但受 P0 阻断；`0086/0087/0089` 已建立 v1 taxonomy、executor annotation/hash、release leaf 校验，以及候选的固定 `qbank_control_definer` 18-function/14-relation handoff 和迁移 CLI 低权 post-flight。候选 handoff 按对象精确 allowlist 收敛 function、表级/列级和 global/`public` default ACL；`qbank_curator`/`qbank_cache_epoch` 是唯二刻意允许 request 只读的非敏感表，所有既有 generation 分区/索引 owner 也纳入目录检查和移交。**但 bounded reader、security-definer view、词法 helper 和 pool/cache/epoch trigger 未进入同一 owner/ACL/RLS/fixed-`search_path` 目录闭包；移交后控制摄取或 app retrieval 可能 42501。** `RAG-FUNNEL-01A` 先冻结闭包，真实隔离 PostgreSQL 组合根才有意义；目前不能声称标准部署已验证。它们不是 `MetadataReviewReceipt`，也没有 generation projection 或 serving filter。Worker 仍以固定“技术岗”启动，未使用 job metadata 形成 route。 |
...
- [ ] `RAG-FUNNEL-01A`：先冻结 sealed dependency manifest，覆盖 control writer/trigger、bounded app reader、它们的调用 helper、security-definer view、pool/cache/epoch trigger 及所有所依赖受管 relation；逐对象规定 owner、精确 app/control/PUBLIC ACL、RLS entry、固定 `search_path`、移交/重放与 post-flight catalog 断言。验收在完整迁移低权组合根中先证明 control ingest/pool promotion 和 app active-metadata、dense、lexical、distance、evidence、question-evidence、cache-epoch 均可用且 raw relation/view read=0；不允许以 app raw-table grant 或旧 migration owner 规避。
...
| 子项 | 对应登记 | 状态 | 必须完成的事 |
| --- | --- | :---: | --- |
| 全双工语音 | `PRD-TEST-006` | ☐ | 将 same-origin streaming ASR、turn-taking、浏览器取消、成本、删除、外送 attempt 与真实 browser→API→provider E2E 接到生产组合根。当前批量 ASR + TTS 不改称全双工。 |
