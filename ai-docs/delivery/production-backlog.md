...
| **图片简历 OCR（光学字符识别）** | 🟠 **合同缝已接线，生产仍 disabled**：`resume.ocr.v1` 经 typed binding + `SealedOcrProvenance`（身份封印，非出站 host pin）；面试缺 binding 则画像不可用、零视觉外呼。`OCR_ENABLED=1` 仍拒绝 API 组合根（binding 存在也不开），缺失或非 `1` 返回 `422 image_ocr_unavailable`，不创建额度、恢复工件或模型调用。旧的 `ocr:prove` 是脚本模型/计费路径证据，不能证明当前启用或百炼视觉能力。生产恢复仍须媒体预算、attempt/unknown、删除与合成 fixture 的脱敏组合根。 |
