...
**本切片**：**EG3 / 题域 isolation product face closed under authorize**（`domainIsolationClosed=true` · `eg3ProductClosed=true`）；**R4·FUNNEL product-close reassess honest flip under authorize**（`r4ProductClosed=true` · `funnelProductClosed=true` · coveredCount **8** Ban invent · `productCloseFlipped=true` · Ban假关）；**EG4 / wrong-track product face closed under authorize**（`eg4ProductClosed=true` · `wrongTrackProductClosed=true` · Ban flip gR45/r4/funnel · Ban empty meta）；**R4/FUNNEL + EG3 + EG4 product faces closed under authorize** · **G-R4-5 STILL OPEN**（`gR45Closed=false` · default do NOT auto-flip）· Ban invent coveredCount · Ban wash ARCHIVE/Batch4b/EG3/R1/EG4 evidence/rem·SSOT·EXPLICIT · Ban self-nail `post_prove_dual_pass` · **partial scoped retrieve alone ≠ flip**。
