...
**本切片**：**EG3 / 题域 isolation product face closed under authorize**（`domainIsolationClosed=true` · `eg3ProductClosed=true`）；**R4·FUNNEL product-close reassess honest flip under authorize**（`r4ProductClosed=true` · `funnelProductClosed=true` · coveredCount **8** Ban invent · `productCloseFlipped=true` · Ban假关）；**R4/FUNNEL product face closed under authorize** · **G-R4-5 STILL OPEN**（`gR45Closed=false` · default do NOT auto-flip）· Ban invent coveredCount · Ban wash ARCHIVE/Batch4b/EG3/R1/rem·SSOT·EXPLICIT · Ban self-nail `post_prove_dual_pass` · **partial scoped retrieve alone ≠ flip**。
