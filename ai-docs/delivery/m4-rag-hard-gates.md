...
| 面 | 代码位置 / 行为（保留至切流批准） |
|----|----------------|
| **Worker「技术岗」硬编码（R1）** | 静默硬编码已收口：`adaptive-role-resolve.ts` + `MEETWISE_TECH_ROLE_FAIL_CLOSED`（默认 off=legacy「技术岗」；on=缺 route → `adaptive_role_route_missing`）。`main.ts` 不再注入 `role: '技术岗'`。**默认仍非通用出题就绪**；R2 **wire 已齐** / **structural CLOSED**（authorize+prove）· **R2 NOT closed** as HA/suite/verbal/controlPlane/R4/FUNNEL（≠ 路由已生效）；不宣称 R4。 |
...
**GAP-RAG-01**：评测 harness + prove **可跑**（`harness/r1-tech-role-fail-closed.md` · `eval/r1-tech-role-fail-closed.eval.md`）；**R1 仍未关**（prove 绿 ≠ closed）；R2 **生产闭环 wire 已齐**（P-MODEL…P-START/P-FAKE dual-passed；G-R2-5）；**structural CLOSED**（authorize+prove）· **R2 NOT closed** as HA/suite/verbal/controlPlane/R4/FUNNEL（≠ 路由已生效）；**不宣称题域隔离已关（R4）**。等 `mw-rag-route` 审评测集。
