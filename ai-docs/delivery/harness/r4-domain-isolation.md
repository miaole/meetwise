...
| 声明 | 裁定 |
|------|------|
| **R4 是否已关？** | **否（pass ≠ R4 / ≠ HA / ≠ cutover / ≠ suite）。G-R4-5 aggregate product face closed under authorize · gR45Closed=true · live canHonestlyFlip。** EG3 / 题域 isolation product face **closed under authorize**（`domainIsolationClosed=true` · `eg3ProductClosed=true`）· R4/FUNNEL product face **closed under authorize**（`r4ProductClosed=true` · `funnelProductClosed=true`）· EG4 / wrong-track product face **closed under authorize**（`eg4ProductClosed=true` · `wrongTrackProductClosed=true` · knife `g-r4-5-eg4-wrong-track-product-close`）· EG5 / product SSOT product face **closed under authorize**（`eg5ProductClosed=true` · `productSsotFlipped=true` · knife `g-r4-5-eg5-product-ssot-product-close`）· EG6 / MS3≠R4 product face **closed under authorize**（`eg6ProductClosed=true` · `ms3EqualsR4Closed=false` retained · Ban claim R4 closed from MS3 · no invented twin · knife `g-r4-5-eg6-ms3-ne-r4-product-close`）· EG1 / dual-claim product face **closed under authorize**（`eg1ProductClosed=true` · `gR45DualClaimClosed=true` · knife `g-r4-5-eg1-dual-claim-product-close`）。本 harness 仍钉「≠ G-R4-5 all closed · ≠ invent coveredCount · ≠ wrong_track=0 invent · MS3 ≠ R4 · Ban flip gR45Closed」；**不得**把 prove EXIT=0 写成 G-R4-5 all closed |
