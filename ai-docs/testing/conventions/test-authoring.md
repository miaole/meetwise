...
行走骨架的总是门是 `pnpm regression --core`：`docs:check`、`arch`、`db:prove`、`runtime:prove`、`graph:prove`、`pipeline:prove`、`api:validate`、`api:smoke`。按触达面加跑的隔离 prove 列在 [run-gates](../../skills/testing/run-gates.md)。**不要**再引用过时的「5 个 gate / 52 断言」计数。每条可复跑断言必须**回挂到一条业务用例的某条 flow**；未挂上的断言要么补用例，要么删除。范例见 [UC-interview-submit-answer](../../requirements/use-cases/UC-interview-submit-answer.md)。
