...
| 机制 | 代码位置 | 实际保证 | 不保证 |
| --- | --- | --- | --- |
| Owner 枚举排序 | `gateway_dispatch_owners('interview')`（迁移 `0128`） | 只返回 owner id；按该 owner 最老可领取/过期 running 行的 `created_at`、再按 owner id 排序。本树的 `0124` 是已合入 main 的 RAG ACL，不是本切片 | 不是公平锁、不是容量预留 |
...
- 确定性：`pnpm interview-dispatch:unit:prove`（无数据库）。
- 隔离 PostgreSQL：`pnpm interview-dispatch:prove`（`releaseEvidence=false`，不能代替发布或云多副本演练）。本环境若无 Docker，该命令不能作为本切片的通过证据。
