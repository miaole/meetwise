...
> **实现现状（诚实校准，2026-08-13）**：**已接线跑通**的是**文本/粘贴 + 结构化 + PII（个人可识别信息）闸（NFKC 归一 + 行内/+86/全角脱敏 + ≥11 位数字 fail-closed 兜底）+ 注入拦截 + `pgp_sym_encrypt` 静态加密 + HMAC（带密钥哈希消息认证码）去重 + 复合 FK（外键）同 owner + RLS（行级安全）越权=0**。图片简历 OCR 当前不属于已接线路径：在 `MODEL-OP-01` 提供 typed binding、原生凭据/endpoint、媒体预算、attempt/unknown、删除和合成 fixture 的脱敏组合根前，`OCR_ENABLED=1` 会拒绝 API 组合根，缺失或非 `1` 的开关在入口前返回 `422 image_ocr_unavailable`，不会预留额度、创建恢复工件或调用模型。旧 `pnpm ocr:prove` 的脚本模型结果仅用于定位历史计费链，不能证明视觉能力、浏览器上传、扫描型 PDF（便携式文档格式）逐页 OCR、完整删除或供应商保留期。PDF 文本层/docx 抽取适配器、版面感知分区、完整 provenance span（来源定位范围）、多模态/GitHub 印证仍属后续能力。
