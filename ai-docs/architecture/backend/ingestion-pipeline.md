...
> **实现现状（诚实校准，2026-07）**：**已接线跑通**的是**文本/粘贴 + 结构化 + PII 闸（NFKC 归一 + 行内/+86/全角脱敏 + ≥11 位数字 fail-closed 兜底）+ 注入拦截 + `pgp_sym_encrypt` 静态加密 + HMAC 去重 + 复合 FK 同 owner + RLS 越权=0**（`pnpm resume:prove`，经 3 轮安全审计）。**未接线/桩**：① **图片简历 OCR(qwen-vl) 仍是桩**——`resume.service` 命中图片返回**显式 `422 image_ocr_unavailable`**（可解释、引导改传 PDF/Word/粘贴文本，**不静默吞**），非已落地能力；② PDF 文本层/docx 抽取适配器层为本期待补（骨架先行 text）；③ 版面感知分区 + provenance span 完整、多模态/GitHub 印证仍属 Phase 1/2 未建。
