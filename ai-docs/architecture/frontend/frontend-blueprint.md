...
前端订阅的是**业务事件**（`progress`、`question_ready`、`waiting_user`、`answer_evaluated`、`session_concluded`、`report_ready`、`report_unavailable`、`interview_unavailable`、`error`），不是模型 token（见 `langgraph-blueprint.md`）。`session_concluded` 只携带练习控制流的 `early_weak`/`thrashing` 预览，**不是**终态，不得映射为 lastScore/overall/band（见 `interview-signal-sse.md`）。`answer_evaluated.score` 只经 `practiceHintScore`（canonical question identity **加** answer claim）展示练习 hint；缺身份不展示分（不是 0）。`report_ready.overall` 非 0..100 整数不写入视图。这是 `SCOR-00H` 消费诚实，不是测量权威。`interview_unavailable` 是出题 fail-closed 与 job 死亡的终态，前端必须停止转圈，不得等待下一条假 `question_ready`。封装成一个类型化 async-generator hook，全站只有这一处 SSE 解析逻辑：
...
- GitHub Pages 只发布 `docs/` 预览版项目展示（仅面试练习，不是求职者/面试官双角色受众；架构亮点、合成截图、源码/README 入口；必要说明只在页脚；招聘不在本预览范围），不承载 Next Server Actions、cookie、API、SSE 代理或用户数据。截图从 `apps/web/docs/screenshots/` 在发布时复制，必须标明合成；不得把支付、完整删除写成已开放，也不得把 OCR/语音写成“未开放不能用”。
