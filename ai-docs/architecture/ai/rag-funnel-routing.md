...
`0086`、`0087` 与 `0089` 只完成了这张图最前端的一小段控制面基础：v1 taxonomy、executor annotation/hash 的写入形状、release leaf 校验，以及候选的固定 `qbank_control_definer` 18-function/14-relation handoff 与迁移 CLI 低权 post-flight。当前候选 handoff 按每张表的 request/control 最小 privilege manifest 收敛函数、表级和列级 ACL，并检查 global 与 `public` schema 的 function/table default ACL；它还递归处理已有 `qbank_generation_chunk_*` 分区及索引 owner，启动门会拒绝 owner、PUBLIC/未知 grantee、列授权和 default-ACL 漂移。`qbank_curator` 与 `qbank_cache_epoch` 是清单中刻意允许 request 只读的非敏感元数据，其余受保护表均不向 request 授予原始读取。

但该 handoff **不是可部署闭包**：bounded app retrieval readers、`qbank_retrieval_candidate` security-definer view、词法 helper，以及 pool/cache/epoch trigger 仍未在同一 owner/ACL/RLS/fixed-`search_path` manifest 中声明和验证。移交底表后，这些对象可能以旧 migration owner 访问而 42501；不得靠重新开放 app raw-table privilege 暂时打通。`RAG-FUNNEL-01A` 先冻结并验证此目录依赖闭包，才允许把候选 handoff 作为 `RAG-FUNNEL-01` 的部署前置。它尚无真实隔离 PostgreSQL 组合根回执，本机 Docker daemon 不可用，故不能称标准部署 provisioning 已验证。该 annotation 没有 reviewer、policy、撤销或可验证 receipt，不能称为人工审核；历史本地回执均为 `releaseEvidence=false`。它们不提供已验证的 owner manifest handoff、generation projection、track-local ANN/词法/RRF/evidence、job 自动分类、route snapshot、embedding compute cache 或同桶 fallback。因此这张图仍是目标态，不能据此称题域隔离已经 serving。
