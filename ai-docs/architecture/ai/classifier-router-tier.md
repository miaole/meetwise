...
| RAG 题域路由（岗位/自由文本） | 规则→便宜 LLM 严格枚举→服务端校验 | 低置信或 unknown 标为 route unresolved、要求补充输入；**不得**兜底全量检索或让用户选桶 |

## 3. 三个硬点

1. **延迟预算**：语音热路径多一次 LLM 往返就爆。拦截器必须**本地快分类器**，不是 LLM 往返。
2. **fail-mode 按用途分**：安全 **fail-closed**（拦不准就拦死）；已授权、单域的纯任务路由可按预定义默认分支降级；会扩大题库/工具/数据范围的多域路由则 **fail-to-clarification**，不能把“默认”解释成全量检索。同一个分类器在不同用途下失败语义相反。
3. **级联**：便宜分类器先过滤容易的 ~95%，仅**低置信**才升级到强模型裁决。moderation 与路由都用这招——成本最优。

## 4. 可观测 + 可测

- 分类/路由是有状态决策：**misroute 是失败模式**，进对抗测试集。岗位题域的具体快照、SQL 硬过滤、证据二次验证，以及仅 clean 同桶无题才允许的一次 LLM fallback 和成本/unknown 合同见 `rag-funnel-routing.md`；当前它们尚未接线。
