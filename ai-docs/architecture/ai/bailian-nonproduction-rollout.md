...
| 视觉 OCR | `VISION_MODEL_NAME` 加受控 OCR binding（未实现） | `MODEL-OP-01` 前所有环境均不得以用户图片/原始输出执行 OCR smoke；`OCR_ENABLED=1` 会拒绝 API 组合根，手动 `vl:smoke` 也固定 fail-closed、不会读取凭据/图片或发网。恢复前必须具备合成 fixture、脱敏回执和 typed binding。 |
