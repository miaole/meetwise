...
现有删除、撤回和外部回执未闭合前，禁止把上述原文、摘要或 embedding 写成跨会话生产数据。0125 只把 `vector_chunk.kind='memory'` 纳入账户删除回执（sink `memory_vector_chunk`），**不是**完整记忆删除；`user_memory` / trace / 外部副本仍按 `architecture/ai/privacy-deletion-sink-inventory.md` 未闭合。完整逐项任务、勾选和关闭证据见 `delivery/production-readiness-remediation-register.md` 的 `MEM-00` 至 `MEM-14`。
