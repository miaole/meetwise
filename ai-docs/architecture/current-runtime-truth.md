...
| `pnpm privacy-erasure:http:prove` | 2026-08-10 在当时 78 个迁移后的隔离 HTTP（超文本传输协议）通过 `4/4`，`releaseEvidence=false`。当前 main 经 `0128` 共 128 个迁移；`0124` 已在 main（RAG ACL，不是删除入口）。#74 落地 `0126` 答题双写互斥；#75 落地 `0127` 预览 OCR binding；#77 落地 `0128_interview_dispatch_fairness`，均不改公开删除入口。503 路径源码现有 8 个会执行的断言（登录令牌 503、重放 503、隐私 JWS 当 Bearer=401、合法登录+JWS 头仍 503、无 `/answers`、request/target=0），休眠 202 套件不得跑。本环境 Docker 不可用，不得填写新回执。 | 真实 `AUTH_SECRET` 登录令牌、重复 DELETE、以及把隐私 JWS 当 Bearer 都不能打开公开删除，也不建 request/target。它证明公开入口仍暂停，不证明任意删除完成，也不证明签发器已接线到 HTTP。 |
