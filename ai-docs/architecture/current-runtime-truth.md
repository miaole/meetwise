...
本分支已跑绿（无 Postgres）：`pnpm question-generation-fail-closed:prove`（含 leftover citation 丢弃 vs 有检索未知 ref fail-closed）、`pnpm native-fail-closed:prove`、`pnpm adaptive-graph:prove`、`pnpm adaptive-grounding:prove`。`native-fail-closed:prove` 在赋值 proof key 前删除 ambient `DASHSCOPE_*_API_KEY_FINGERPRINT` / `DASHSCOPE_REVOKED_KEY_FINGERPRINTS` 以及 leftover `DASHSCOPE_*_URL` / `DASHSCOPE_COMPAT_BASE_URL`，避免 `pnpm regression` 注入根 `.env` 后把超时/畸形误判成 `*_fingerprint_mismatch` 或 `dashscope_native_endpoint_env_forbidden`；生产指纹与 URL 围栏不关。`pnpm adaptive-degrade:prove` / `pnpm adaptive-life:prove` / `pnpm adaptive-flow:prove` 需隔离 PostgreSQL：**只用 remote DB env 或 GitHub CI 隔离库，禁止本 agent 起本地 Docker Postgres**。GitHub Actions `verify`（run `33851407093`，tip `22bde9b`）已重跑 `adaptive-degrade:prove`（含拆开的 eval/generate AbortSignal 计数）为通过；这不是本机 remote-DB 回执文件，也不是发布证据。旧 2026-08-10 degrade 回执已过期。多轮脚本模型必须每轮发出不同题面：critique 判 duplicate 现在 fail-closed，不再发明替补题。均 `releaseEvidence=false`，不证明真百炼或发布。
...
| `pnpm question-generation-fail-closed:prove` / `pnpm native-fail-closed:prove` / `pnpm adaptive-graph:prove` / `pnpm adaptive-grounding:prove` | 本分支本地跑绿（无 Postgres），`releaseEvidence=false` | 错误码分类、原生缺 Key/超时/畸形（含零向量/空 rerank）、图 fail-closed、grounded 模板、embedder miss 不发明 fundamental 题。`native-fail-closed:prove` 自清 regression dotenv 指纹/撤销清单/旧 URL 变量，不关生产围栏。不证明真百炼、隔离 lifecycle 或发布。 |
...
| `pnpm memory-governance:prove` | 源码与迁移 `0093` 已在树；隔离 PostgreSQL 须 `run-e2e-isolated`。本环境 Docker 缺失 → `blocked:docker_daemon_missing`，不得填通过回执。`releaseEvidence=false`。 | MEM-00 治理：同意/epoch、事实准入、两阶段召回、generation/snapshot、复用 0091 账户删除链与 app_role 拒权。不证明长期语义记忆已上线或控制面已关。 |
| `pnpm memory-control-surface:prove` | 源码与迁移 `0107` 已在树；隔离 PostgreSQL 须 Docker。本环境不可用，不得填写通过回执。`releaseEvidence=false`。 | MEM-10 命令层七矩阵（纠正/撤回/删除/幂等/RLS/lease）。不是 MEM-00 关闭证据，也不开放生产长期写入。 |
| `pnpm mem00-int00:prove-path` / `pnpm mem00-int00:prove-path:gate` | 2026-09-09 助手主机：可移植 pin 绿；隔离项因无 Docker daemon 记 blocked；gate 静态诚实边界绿。回执 `.tmp/mem00-int00-prove-path/*`（gitignored），`releaseEvidence=false`，`controlPlaneClosed=false`。 | 只编排 MEM-00 / INT-TRANSCRIPT-00 证明入口并诚实分类结果；**不**关闭控制面、不改生产 DELETE、不开 INT-TRANSCRIPT-01 生产 write。 |
...
| `pnpm memory:prove` | 历史：2026-08-10 在 64 个迁移后的隔离 PostgreSQL 通过 11/11，回执 `2026-08-10T10-08-16-315Z-82261-1b450349-dd23-4396-aa64-016c36ea8309.json`，`releaseEvidence=false`。当前树迁移已到 130；本环境 Docker 缺失，2026-09-09 prove-path 重跑记 blocked，不得用历史回执作当前通过。 | episode（情节记忆）精确判重、历史弱项只读投影、RLS（行级安全）跨主体读取为 0，以及题面记忆不含手机号；不证明语义长期记忆、完整删除权、MEM-00 控制面关闭或云端数据面。 |
