...
这同样不是完整删除权证明：`0096` 已为 event/report/`ai_graph_run` 补 **DB rehearsal** 物理 purge 与 sink receipt，但公开删除仍 503，worker 未改走 `0091`，vector/trace 已诚实登记为无 interview 作用域 resolver。派发先赢的供应商调用尚无删除/保留回执，且备份、cache（缓存）、对象存储、`user_memory`（用户记忆）、完整简历/B 端数据仍未闭环。外部 target（目标）无 receipt 时请求保持 `pending_external`（等待外部完成）。`INT-TRANSCRIPT-00` 已有本地签发器、target/receipt 账本和 submission/receipt 合同，但公开删除仍 503，真实 HTTP/SSE/RLS 组合根与部署密钥注入尚未证明；`INT-TRANSCRIPT-01` 的真实用户 write route 因此保持 blocked。必须继续做受控工件的历史 checkpoint 枚举/删除、逐 sink（数据落点）回执和真实数据面验证，再讨论压缩、完整面试记录或长期记忆。
...
| `pnpm privacy-erasure:http:prove` | 2026-08-10 在当时 78 个迁移后的隔离 HTTP（超文本传输协议）通过 `4/4`，`releaseEvidence=false`。当前树 123 个迁移；503 路径源码现有 8 个会执行的断言（登录令牌 503、重放 503、隐私 JWS 当 Bearer=401、合法登录+JWS 头仍 503、无 `/answers`、request/target=0），休眠 202 套件不得跑。本环境 Docker 不可用，不得填写新回执。 | 真实 `AUTH_SECRET` 登录令牌、重复 DELETE、以及把隐私 JWS 当 Bearer 都不能打开公开删除，也不建 request/target。它证明公开入口仍暂停，不证明任意删除完成，也不证明签发器已接线到 HTTP。 |
