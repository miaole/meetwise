...
| `pnpm privacy-erasure:http:prove` | 2026-08-10 在当时 78 个迁移后的隔离 HTTP（超文本传输协议）通过 `4/4`，`releaseEvidence=false`。当前 main 经 `0129` 共 129 个迁移；`0124` 已在 main（RAG ACL，不是删除入口）。#74 落地 `0126` 答题双写互斥；#75 落地 `0127` 预览 OCR binding；#77 落地 `0128_interview_dispatch_fairness`；#82 落地 `0129_privacy_erasure_preview_path`，均不改生产 DELETE。当前树另有预览路径断言（缺幂等键 400、预览 202 回执未完成、同键重放、GET 回执、生产 DELETE 仍 503）。503 路径源码现有 8 个会执行的断言（登录令牌 503、重放 503、隐私 JWS 当 Bearer=401、合法登录+JWS 头仍 503、无 `/answers`、request/target=0），休眠生产 202 套件不得跑。本环境 Docker 不可用，不得填写新回执。 | 真实 `AUTH_SECRET` 登录令牌、重复 DELETE、以及把隐私 JWS 当 Bearer 都不能打开**生产**删除。预览路径是另一条账本，回执固定 `preview_incomplete`。不证明签发器已接线到生产 HTTP，也不证明跨存储删除 SLO。 |
