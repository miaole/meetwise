...
| C 端进度投影 | 成长主页「已答题数」、`/interviews` 与详情进度文案 | `GET /interview` 的 `issued_turns`/`answered_turns` 与 `GET /profile/overview.answered` 都从 privacy-active 的 `interview_question`（题目账本）投影：`status='answered'` 计入已答，`issued`/`queued`/`cancelled` 不计已答。`interviewsByStatus` 同样只计 privacy-active 场次。`avgScore` 与成长档案 `totals.answered`（文案「累计已评分」）仍只读可评分 ScoreCard。题目账本不是测量质量、B 端排序或用途授权的事实根。成长主页取数失败渲染「—」，不把缺失伪装成 0。 | 代码已接线；断言写入 `pnpm api:validate` 与 `pnpm web:prove`。不证明浏览器实操、overview 擦除 HTTP 或发布。 |
