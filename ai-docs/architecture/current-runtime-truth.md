...
| `pnpm runtime:isolated:prove` | 通过，37 个隔离 PostgreSQL（关系型数据库）断言，其中 7 个为执行时限/迟到成功/prepare（预派发准备）/非法等待预算回归 | 永不收口的已派发模型调用在 35–40 毫秒测试预算内变为 `unknown/model_execution_timeout`；同幂等键重放与双并发均只派发一次，AbortSignal 会被触发，迟到成功不能写工件或 trace。`0128` 把同键 claim 创建用短事务 advisory lock 串起来，孤儿 create-permit 只回 `wait`（不二次 execute、不把 `missing_after_conflict` 当成派发许可）。不证明供应商已取消计费或账单对账。`0126` 答题双写围栏与 `0127` 简历 OCR binding 保持原编号。 |
