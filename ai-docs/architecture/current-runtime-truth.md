...
| Pages | 仓库 `docs/` 是预览版静态项目展示（仅面试练习、架构亮点、合成截图；必要说明只在页脚；招聘不在本预览范围），由 `.github/workflows/pages.yml` 在 `main` 上复制 `docs/` + `apps/web/docs/screenshots/` 发布。它不是应用运行时，不启动数据面，也不再使用已删除的 `preview-site/` 签名目录。公网若仍显示旧「预览环境准备中」文案或求职者/面试官双角色导航，只说明 Pages 尚未切到该 workflow，不能当成应用部署证据。git `main` 现为 `4c515d1`（#92 后，含 #89/#91/#93/#95）。commerce/report 的 130 迁移回执仍钉在产生它们的 CI tip `54cf595`，不是本 tip。 | 已接线待验。静态目录与 workflow 在仓库内可核对；公网切换与 GitHub Pages 环境授权不是本文件的发布证据。 |
...
| 计费与权益 | 预留、调用、确认、失败释放、对账 | 每个可计费调用以幂等键、租约和账本状态防止重复扣费；Redis 锁不能替代数据库账本。 | commerce saga：main CI 130 迁移 50/50（`releaseEvidence=false`）。押题失败退款 / 收割退款：源码已接线，**无 130 迁移 `quiz:prove` / `reaper:prove` 回执**，不得写成当前通过。支付渠道及云故障演练待验。 |
...
- API 组合根经 `createInterviewVoiceSeams()` 接线批量 ASR/TTS：`voice.asr.v1` / `voice.tts.v1` 须 wired 且存在独立能力 Key。Worker `voice-turn.ts` / `interview-voice.ts` 把 `audio → ASR → 图 resume → 下一题 → TTS` 套在同一业务图外，文本和语音共享题目、幂等、权益、checkpoint 和评分边界。未配置必须显式失败，不能把 fake 当线上服务。**流式 ASR** 与服务端 turn-taking 未验证：精确双旗 `VOICE_STREAM_ASR_ENABLED=1` 且 `VOICE_STREAM_ASR_PREVIEW=1` 才请求预览；生产 / enforce / `MEETWISE_PUBLIC_PREVIEW=1` 仍拒绝。双旗+Key 也不把 live stream 接到组合根（`streamAsrConfigured` / `turnTakingConfigured` 恒 false；`voice.asr-stream.v1` 仍 not_wired）。Key 单独不能开 WebSocket。`runVoiceAdaptiveInterview` / `vstream:prove` 只是 fake-seam 合同，不是产品 E2E。`MEETWISE_PUBLIC_PREVIEW=1` 下 `/transcribe` `/speak` `/speak/stream` 仍 503；本切片只额外放行 `POST /interview/:id/answers`。
...
| `pnpm commerce:prove` | 2026-09-04 GitHub Actions `verify`（run `33864913506`，tip `54cf595`）在 **130** 个迁移后的隔离 PostgreSQL 通过 **50/50**（`migrations: applied=130 skipped=0`）。CI 日志中的回执文件名（非本机文件）：`2026-09-04T10-50-53-032Z-9770-89bfb6ae-f1de-4de7-8082-7487d430a65c.json`。`releaseEvidence=false`。本环境无远程 PG env，禁止起本地 Docker；无本机 remote-DB 回执。2026-08-10 的 64 迁移 50/50 只作历史定位。 | 权益的 FIFO（先到期先扣）、幂等预留/确认/释放、并发不超卖、心跳—回收竞态、exactly-once（精确一次）结算账本、同支付流水只入账一次、16 轮确认/放弃终态配对，以及数据库约束拒绝绕过均在 **130** 迁移 schema 上由 CI 隔离库跑过；不证明真实支付服务商回调、云数据库故障恢复或资金清算。 |
...
| `pnpm report:prove` | 2026-09-04 同上 `verify`（run `33864913506`，tip `54cf595`）在 **130** 个迁移后的隔离 PostgreSQL 通过 **31/31**（源码当前 31 条 `A()`；历史 36/36 回执已过期，不是 5 项失败）。CI 日志中的回执文件名（非本机文件）：`2026-09-04T10-51-05-364Z-10706-eda4736c-fde1-4a8b-be36-723878f12e8b.json`。`releaseEvidence=false`。本环境无远程 PG env，禁止起本地 Docker。 | 报告的幂等入队、失败不影响面试、独立重试/退避、租约重领、stale（过期）worker 拒绝、poison-pill（毒丸）隔离、多租户调度、两并发恰一领和 RLS（行级安全）都在 **130** 迁移隐私 trigger 下由 CI 隔离库跑过；不证明真实模型生成质量、外部通知或云端 worker 可用性。 |
| `pnpm quiz:prove` | **无当前 130 迁移回执。** main `verify` run `33864913506` @ `54cf595` **未跑**本命令。2026-08-10 历史隔离回执 25/25（64 迁移，`2026-08-10T10-13-18-828Z-84072-0d097553-c840-4e20-a606-c51a772b6a71.json`）不得写成当前通过。源码现有 22 条 `A()`，未在 130 迁移上重跑前不得填 22/22。本切片只把命令写入 per-push YAML；YAML 不是回执。本环境无远程 PG env，禁止起本地 Docker。`releaseEvidence=false`。 | v61 typed `(resume_id,privacy_epoch)`（带隐私世代的简历引用）押题任务经队列/worker 执行的合同仍在源码；**当前迁移上的通过结论须等本变更后的 verify 回执**。不证明简历删除围栏竞态、真实模型质量或云端数据面。 |
...
| `pnpm reaper:prove` | **无当前 130 迁移回执。** main `verify` run `33864913506` @ `54cf595` **未跑**本命令。2026-08-10 历史隔离回执 28/28（64 迁移，`2026-08-10T10-16-51-919Z-85265-de318d10-063a-480f-a866-2dfc7d9ff058.json`）不得写成当前通过。源码现有 28 条 `A()`，未在 130 迁移上重跑前不得填 28/28。本切片只把命令写入 per-push YAML；YAML 不是回执。本环境无远程 PG env，禁止起本地 Docker。`releaseEvidence=false`。 | 面试与押题过期租约、心跳—收割竞态、失败退款、已结算不倒退的合同仍在源码；**当前迁移上的通过结论须等本变更后的 verify 回执**。不证明多副本云 worker、真实崩溃注入或删除围栏竞态。 |
