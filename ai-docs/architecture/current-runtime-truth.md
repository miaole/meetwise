...
迁移 `0085` 只为当前文本调用建立 canonical logical-node header、不可回收 dispatch slot 与**只覆盖 `claimed → dispatching` 更新**的数据库围栏。工作树候选 `0088` 改为撤销 `app_role` 对 invocation 的 INSERT/UPDATE/DELETE，只开放固定 claim/dispatch/fail-claim/terminalize/reconcile 过程，并以私有 permit trigger 防御 ACL 漂移下的直写；它尚未取得真实低权 PostgreSQL 组合根回执。故不能称 raw SQL 已被完整状态机围住，也不能关闭 `MODEL-OP-00` P0。图内 critique、判重和 quote 证据失败虽已收紧为一次派发后的确定性题面或澄清/未评分，node digest 仍由调用方参与构造，catalog 仍未被 `invoke()` 主链消费。
...
| 岗位题域路由与桶内无题 | Worker 当前固定以“技术岗”启动；检索输入为 competency/difficulty。局部无命中会混入 Web/普通 LLM 出题或 generic fallback | **未接线。** `0086/0087/0089` 仅在 QBank 摄取控制面增加 v1 taxonomy、executor annotation/hash、release leaf 校验和候选的固定 `qbank_control_definer` 18-function/14-relation handoff。当前源码已候选性地按对象最小 privilege manifest 检查 function、表级/列级 ACL、global/`public` default ACL、owner-transfer 后未知 `SECURITY DEFINER` 和既有 generation 分区/索引 owner；`qbank_curator`/`qbank_cache_epoch` 是显式允许 request 只读的非敏感元数据，其他 manifest 原始表不允许 request raw read。**候选 handoff 尚未形成可部署依赖闭包：bounded retrieval reader、security-definer view、词法 helper、pool/cache/epoch trigger 没有在同一 owner/ACL/RLS/fixed-`search_path` manifest 内证明可用，底表移交后可能 42501。** `RAG-FUNNEL-01A` 必须先冻结并验证这个闭包；不得以 app raw-table grant 或旧 migration owner 规避。真实隔离组合根仍未运行，不能把此称为部署验证。它不是人工 `MetadataReviewReceipt`，也不是 serving filter。本机 Docker daemon 不可用，故没有当前树的低权回执。当前没有 application-bound route snapshot，岗位 `job_posting.competencies` 未进入启动图，generation/ANN/lexical/RRF/evidence/cache 也没有 backend/frontend/qa/ai_ml 等 track 硬过滤。当前没有按 scope 的 immutable projection、embedding compute cache、clean `no_eligible_in_scope` 与 degraded/ACL/stale/unknown 的区分，也没有 `QuestionPlan`、同桶 LLM provenance、rubric/score policy 或 B 端评分排除。因此不得把当前 generic 出题称为“桶内无题时安全生成”。目标与验收见 `requirements/use-cases/rag-funnel-intent-routing.md`。 |
...
| 证据 | 向量 chunk 必须映射 approved source（已审批来源）和 qbank artifact；模型只能引用本轮 known refs | 0072 及此前的本地隔离回执只是历史证据：它覆盖题目工件完整性、撤销即整题不可见、辅助裸 chunk 不可提升为题目工件、正文摘要重算/并发/UPDATE/DELETE 逃逸及低权限读取器的当时合同，均 `releaseEvidence=false`。它不能证明 0086/0087/0089 的 taxonomy review、18-function/14-relation definer handoff、其 reader/view/helper/trigger 依赖闭包、scope projection 或 routed serving。当前 handoff 组合根 proof 因本机 Docker daemon 不可用未执行。真实云秘密挂载、审核员后台、全格式 citation（引用定位）与撤回/删除全链路仍未证明。 |
...
这同样不是完整删除权证明：现有事件/报告等投影尚无物理 purge（清除）receipt（回执），派发先赢的供应商调用尚无删除/保留回执，且备份、trace（追踪）、cache（缓存）、对象存储、`user_memory`（用户记忆）、完整简历/B 端数据仍未闭环。外部 target（目标）无 receipt 时请求保持 `pending_external`（等待外部完成）。`requirements/use-cases/expert-long-interview-runtime.md` 的 2026-08-13 独立审查因此将 `INT-TRANSCRIPT-00`（删除授权/receipt）列为 P0 前置，`INT-TRANSCRIPT-01` 在其验证前保持 blocked；它没有被当前 Graph、SSE 或 queue proof 实现。必须继续做受控工件的历史 checkpoint 枚举/删除、逐 sink（数据落点）回执和真实数据面验证，再讨论压缩、完整面试记录或长期记忆。
...
| `pnpm privacy-erasure:http:prove` | 2026-08-10 在 78 个迁移后的隔离 HTTP（超文本传输协议）通过 `4/4`，`releaseEvidence=false` | 真实注册/鉴权后，面试与旧全量简历删除入口都返回 503；不建 request/target。它证明未授权删除被暂停，不证明任意删除完成。 |
