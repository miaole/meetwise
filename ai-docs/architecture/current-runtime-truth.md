...
| `pnpm int-transcript-preview-submit:http:prove` | 隔离 PostgreSQL + 真 HTTP 才能证明账本落点。本环境 Docker 不可用时不得填写通过回执。`releaseEvidence=false`。 | 预览提交落入 0092 submission/artifact，无 plaintext `kind=answer` job。不是 01 cutover，也不是删除已开放。 |
