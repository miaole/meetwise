...
3. **验证门（可测就必填）**：测试计划（含失败/退款/重复/并发/隔离/注入）；**禁伪验收**（只断言 200/只开页/mock 证质量/AI 自评/只 happy path）；验证命令。改完之后的审核 → 测试 → 回归仪式见 `skills/testing/sop.md`（`status: draft`，遵守但不把技能本身写成已证明）。
