...
3. **验证门（可测就必填）**：测试计划（含失败/退款/重复/并发/隔离/注入）；**禁伪验收**（只断言 200/只开页/mock 证质量/AI 自评/只 happy path）；验证命令。改完之后只走 `skills/testing/sop.md`（`status: draft`）；收束公式见 `skills/testing/fail-closed-gate.md`（文档门：审核 ∧ 验证，缺一阻断，作者不得自签）。
