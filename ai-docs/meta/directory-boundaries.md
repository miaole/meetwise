...
- SIGNAL-SSE 预览投影（#84） → `requirements/use-cases/interview-signal-sse.md`。`session_concluded` ≠ SSE 终态 / 等级鉴定 / 发明分，不得另写第二套「信号已上线招聘面」声明
- Pages 预览展示（#78 / #88） → `docs/index.html` + `.github/workflows/pages.yml`。仅面试练习；招聘不在本预览范围。静态展示 ≠ 应用运行时 / 数据面 / 发布证据，不得另写第二套「线上服务已开放」声明
- 预览批量语音 → `architecture/ai/voice-capability-boundary.md`。`createInterviewVoiceSeams` ≠ 生产 SLO；`/speak/stream` 仍不可用，不得另写第二套「语音已上线」声明
- 预览简历图片入口 → `requirements/use-cases/cend-resume.md` + `apps/web/lib/ocr-preview.ts`。`isOcrPreviewEnabled` ≠ OCR 已上线；Live E2E 仍拒 `OCR_FAKE`
...
- 高并发复核骨架（#87） → `architecture/backend/high-concurrency-review.md`。五面接线 + `HC-GAP-*` ≠ 容量 SLO / 集群锁；`HC-GAP-002` / `HC-GAP-004` / `HC-GAP-006` / `HC-GAP-007` / `HC-GAP-011` / `HC-GAP-014` 已关；`HC-GAP-009` 静态门已接线；隔离 `model-slot-bypass:prove` / `quiz-dual-claim:prove` / `runtime:claim-join:prove` 不进 always-on
