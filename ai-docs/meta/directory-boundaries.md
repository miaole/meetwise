...
- 高并发复核骨架（#87） → `architecture/backend/high-concurrency-review.md`。五面接线 + `HC-GAP-*` ≠ 容量 SLO / 集群锁；`HC-GAP-004` / `HC-GAP-006` / `HC-GAP-014` 已关；`HC-GAP-009` 静态门已接线；隔离 `model-slot-bypass:prove` / `quiz-dual-claim:prove` 不进 always-on
