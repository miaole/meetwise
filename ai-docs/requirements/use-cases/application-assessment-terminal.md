...
- 两种情形下：岗位列表、人才库、导出、通知和任何 recruiter API 都不消费或显示可比较的数值分。招聘方复核页（`/recruiter/jobs/:id/applications/:applicationId`）只读该无分状态；C 端「我的投递」也不再渲染申请分数。该 UI 消费门见 `bend-recruiter-architecture-surface.md`，不替代本用例的数据库 hold。
