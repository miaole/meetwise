...
| 面试控制信号（SIGNAL-01 / 0130） | `interview-control-signals.md` | `observeInterviewSignals` 供 `decideNext` 消费 `early_weak` / `thrashing`；`safety_ceiling` 先赢；不是能力等级校准，不关闭 `INT-LEVEL-01`；静态 `adaptive-signals:prove` / `adaptive-signals-graph:prove` 可进可选 always-on；`0130` 同键 claim 串行不是本 hook 的业务证明 |
