...
- **明确不做：** 不宣称 01 生产 cutover；不登记进 `apiContract` / OpenAPI；不写 plaintext `interview_job.payload.answer`；不调用 `claimInterviewAnswer` / 评分 / 模型 / RAG / memory / B 端投影；不开放公开删除；不新增迁移（`0126`/`0127`/`0128`/`0129`/`0130` 已在 main；不占用 0131）；`0129` 预览删除是另一条账本，公开预览下仍 503；`0130` 是 #72 same-key claim join，不是本切片；`INT-LEVEL-SIGNAL-01` 不改本写面；`#79` 预览批量语音不进本受控写 allowlist（公开预览下 `/transcribe` `/speak` 仍 503）；`#83` Web `/resume` 预览图片入口是另一条 OCR 预览面；Web 公开展示站保持只读（本包不加 `/api/interview/:id/answers` 代理）。不跑 `pnpm db:up`。
