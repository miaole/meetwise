...
5. 调用既有 `submitInterviewAnswer`：同键同体回放、同键异体冲突；正文只进加密 artifact。
...
- 原语：幂等键、RLS、题目行锁、隐私 fence。
- 测试：`TC-INT-TRANSCRIPT-PREVIEW-SUBMIT-*`（见下）。治理分母仍 planned/unmapped，本地 proof 不是发布证据。
...
层映射：`逃`/`并`/`刁` 落在 Fastify inject 或隔离 HTTP，不只堆单元。无 Docker 时隔离 HTTP/DB 不得填写通过回执。
