...
| 来源证据 | 现码：`packages/domain/src/adaptive-interview.ts` 的 `decideNext` / `derivedSoftBudget` / `boundedSoftBudget` / `boundedAbsoluteMaxTurns` / `rememberDecision`；图 `plan` 把调用方软预算与绝对杀开关写入 mind，不再夹产品硬顶。`INT-LONG-INTERVIEW-01` 仍要求 time+module blueprint，不以调高杀开关或软预算代替。 |
...
- **绝对杀开关** `absoluteMaxTurns`：平台 runaway/成本护栏，**不是**面试质量政策。默认 `DEFAULT_ABSOLUTE_MAX_TURNS=120`（长时面试档）。允许配置 INT-LONG 长时档位 60/90/120；再高夹到 `PLATFORM_ABSOLUTE_CEILING_TURNS=180`（杀开关自身的上限，防 `1e9`）。外部 `maxTurns=999` 只能涨到该场绝对杀开关，不能无界。
...
| 特 | `TC-INT-LENGTH-01-S1` | 域+图 | 默认绝对=120；`999` 绝对夹到 180、软预算夹到该场绝对；显式软预算 8 不夹到 16；60/90/120 档可配 |
