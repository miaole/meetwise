...
| 崩溃恢复 | 每个 prepare/deploy/finalize/revoke 入口在取得同一 root-owned `flock` 后，先对 ledger、`current` 软链和公开 manifest 做协调。ledger 与公开 manifest 均采用“临时文件写入 → fsync 文件 → rename → fsync 父目录”；不得将单次 rename 描述为持久化。若发现 `publishing`、manifest 与 ledger fingerprint/release 不一致、或有效公开 manifest 缺少一致 `verified` ledger，则先以**实际公开 manifest**签发撤链记录并等待 Pages 禁用回执；回执前不切换 `current`、不激活候选、不中止到另一 release。公开 manifest 缺失或不可验时，协调器先关闭 Funnel 边缘并停止候选，再保持失败关闭；人工处置前不继续发布。 |
...
`PreviewWebRelease` 状态为 `idle / failed / revoked → staged → active_unpublished → publishing → verified → revoked / failed`。单一 root-owned `flock` 覆盖协调、撤链、构件验签、候选、激活、Funnel、预签名黑盒验证和签名；黑盒 receipt 成功且签名前后都重验相同活动 release 后，先持久写入 `publishing`，才允许原子写入公开 signed manifest；随后才进入 `verified`。候选 release 必须先以独立回环端口证明其 release marker、构建摘要和允许页面，且 systemd 确认整个候选 cgroup 已退出后才可能激活。同一 release digest 重放仅接受完全相同 archive；任何签名、HTTPS、精确 origin、边缘/内部健康、构建摘要、路径或方法门失配均进入 `failed`，恢复前一 release，Pages 链接保持禁用。`publishing` 不是不可恢复状态：进程重启或状态不一致时只允许完成一致性确认，或先收到撤链回执后失败关闭。若 `publishing` 或 `verified` 的任一步失败，必须先签发撤链记录并等到 Pages 禁用回执，再恢复前一 Web。撤链回执使用签名 manifest 的 canonical JSON SHA-256，避免格式化差异。
...
| 异常 | `TC-ecs-public-preview-web-ingress-01-E1` | 构建产物、archive 成员、候选 marker/cgroup 停止、Nginx 校验、切换、外部 HTTPS、公开 manifest 后的 ledger transition 或健康任一失败时，先完成撤链回执，再恢复前一 release；Pages 链接保持禁用。覆盖 manifest rename 后至 `publishing → verified` 前崩溃、撤链 manifest 写入后至 ledger 转换前失败和 ledger 写入失败。机制：带 fsync 的 `publishing` 状态与失败关闭发布状态机。 |
...
| 复杂 | `TC-ecs-public-preview-web-ingress-01-E5` | Web 重启、候选切换回滚、边缘暂时断开、release 过期或撤销时，旧实例可恢复且入口/Pages 均不可指向失效 release；已启用入口先收到 Pages 禁用回执才能切换。覆盖重启时 `publishing`、公开 manifest 已持久而 ledger 回退为 `active_unpublished`、以及 `verified` manifest/fingerprint 不一致；协调完成前新 release 的 `current` 不得生效。机制：systemd restart、带 fsync 的串行 ledger、公开 manifest 对账与 Pages 清单。 |
