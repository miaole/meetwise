...
- **数据库影响：** `resume.source_kind` 允许 `image`；`resume_profile.ocr_binding`（迁移 0127。`0124_rag_retrieval_acl_fail_closed` 已在 main；开放 PR #70=0125、#74=0126，不得改号）存无 PII/无原文的 binding 快照。OCR 恢复密文仍只活在 `resume_ocr_artifact`。`releaseEvidence=false`。
