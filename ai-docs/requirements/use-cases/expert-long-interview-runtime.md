...
| `INT-P1-BLUEPRINT-ROUTE` | P1 / open | 短流程图已按覆盖/证据/会话信号收口（`UC-INT-LENGTH-01`，安全天花板 16，不再是固定八轮硬顶）；worker 仍默认“技术岗”。没有 60/90/120 分钟、module、deadline、route/rubric/model/prompt snapshot。 | `INT-LONG-INTERVIEW-01` 必须在 `RAG-FUNNEL-03/04` 和版本化评分后独立审查；01 不靠加大固定轮数交付，不接 RAG/Web。短流程长度政策见 `requirements/use-cases/adaptive-interview-length.md`。 |
