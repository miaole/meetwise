...
| 来源证据 | `0096` 登记过 MEM-00 未覆盖 `vector_chunk`；`0093` `memory_begin_account_erasure` 只枚举 3 sink；`memory_embedding` 只删 `memory_index_generation`；0124 补 `memory_vector_chunk`；PRD-TEST-015 / 盘点仍开 `user_memory` 与 trace；公开入口 `PrivacyService` 503。 |
...
| 数据库影响 | 迁移 `0124`：扩展 sink CHECK、worker ACL/RLS、begin/claim/purge、写围栏触发器。 |
...
- **异常流 Exception：**
  ...
  - **E6 重连：** 同 key 重放同一 `requestId`。0124 只建 1 个 target，purge 同事务把 request 置 `completed`；再 purge → `memory_vector_chunk_target_request_not_active`（证明层断言拒绝，不是再删 0 行）。SQL 虽有「target 已 erased 且 request 仍 active → `deleted_count=0`」分支，单 target sweep 不可达，不得当成本迭代验收。
