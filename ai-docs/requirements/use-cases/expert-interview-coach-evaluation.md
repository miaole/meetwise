...
| 事项 | 当前证据 | 可得结论 | 不可得结论 |
| --- | --- | --- | --- |
| 确定性评分完整性 | 有真实 DB、invoke、LangGraph、lifecycle、HTTP→队列 proof | quote 校验、一次受限 repair、同答重放、clarify/unscored 与权益隔离的代码路径已覆盖 | 真实模型理解答案的能力 |
| 真模型相关性分片 | `3/3` 跑题/指代样本为 `relevant=false, score=0`，Wilson 95% 下界 `0.438` | 这是小样本信号 | 泛化相关性，发布通过 |
| quote repair 分片 | `2` 次 repair 中成功 `1`、耗尽 `1`；成功率 `1/2=0.500`，Wilson 95% 下界约 `0.095` | 失败会被安全地澄清/拒绝，而非写入高分 | repair 可用性达标 |
