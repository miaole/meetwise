...
| --- | :---: | :---: | :---: | :---: | --- |
| SCOR-00 关闭公开伪评分旁路 | ☑ | ☑ | ☑ | ☐ | `2026-08-13` 的 `pnpm scor-00:http:prove` 已以完整迁移、独立低权 runtime login 的真实 HTTP 证明 `/answer` 与旧公开 contract 不写事件，C/B、重放、并发、跨主体调用后的事件/消费/队列/报告/assessment/B 端申请增量均为 0。该本地回执 `releaseEvidence=false`；原始 event 写权限分离、rubric/measurement 版本属于 `SCOR-01/02`，不得在本项虚报完成。 |
| SCOR-01 版本化 rubric 与两阶段事实根 | ☑ | ☐ | ☐ | ☐ | 先经 `INT-TRANSCRIPT-00/01` 取得 canonical artifact/删除 receipt；issue 阶段冻结题目/rubric/policy/cohort，submission 阶段追加 answer version/ScoreRequest，二者与 ScoreCard 同一受控事务绑定；原地 UPDATE/DELETE 拒绝。 |
| SCOR-02 确定性聚合、writer 与消费迁移 | ☑ | ☐ | ☐ | ☐ | 专用 score-writer/permit/verifier 与全部 C 消费面同包切换；只消费 `practice_eligible` 或经批准 `b_review_eligible` ScoreCard，任意无资格状态或 legacy event 的聚合数为 0。 |
