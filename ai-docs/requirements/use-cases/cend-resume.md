...
- **后置**：成功才导航；失败停留表单。Web 不写 ConsumptionRecord；账本仍由 API `ocr:prove` 负责。本切片无新迁移（`0127`–`0130` 已在 main；下一空号 ≥`0131`）
