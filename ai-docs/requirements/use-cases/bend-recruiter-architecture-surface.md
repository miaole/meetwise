...
name: 用例 · B 端招聘方面板：架构说明与申请复核消费
description: 招聘方/面试官在产品内用人话看到承重设计，并打开岗位申请复核页；只消费已授权的最小状态，不恢复数值分或逐题原文。
type: use-case
scope: bend
level: spec
status: active
...
>
> 生成前门禁：任务范围=B 端 `/recruiter/how-it-works` + `/recruiter/jobs/:id/applications/:applicationId` + 列表入口 + 申请分数消费止血；来源=`application-assessment-terminal.md`、`current-runtime-truth.md` B 端行、现网岗位/人才库只有表格无复核路径、C 端「我的投递」仍可能渲染 `application.score`；明确不做=Pages 重写、新迁移、新评分 API、人工复核工单、数值排序、自动录用；领域对象=`JobPosting`/`JobApplication`（只读投影）；状态机影响=无；契约影响=无（复用既有 list）；数据库影响=无；验证=`pnpm web:prove`、`pnpm -C apps/web prove:public-copy`、`pnpm docs:check`。

## 落地范围

| 字段 | 结论 |
| --- | --- |
| 任务范围 | 招聘方登录后能读懂「下一题跟着答、进度在服务端、假分入口已关、两边记账分开、检索有权限」；并能打开某一申请的复核页，只见状态/来源/岗位能力，不见分数和面试原文。 |
| 来源证据 | `UC-ASSESSMENT-001` 已把 B 端收口为 `assessment_unavailable` + `score=NULL`；`listJobCandidates`/`listTalentPool` 查询边界置空 score；招聘方列表行不可点、无复核页；C 端 `/jobs` 仍写 `评分 {app.score}`。 |
...
| 接口契约影响 | 无新 endpoint。复核页并行读既有 `GET /recruiter/jobs/:id` 与 `GET /recruiter/jobs/:id/candidates`。 |
| 数据库影响 | 无迁移。 |
| 测试计划 | 纯函数：非法 `applicationId` 不得命中、score 恒不渲染、状态文案无死胡同。文案门：架构页用人话覆盖七项能力，禁止 Grok/自动录用/把本地门禁写成发布。浏览器层沿用既有 C→B 规格，并断言复核入口可见且无「综合评分/我的回答」。 |
...
| `PENDING-PRD-REVIEW-01` 人工复核工单 | 无案件、领取、四眼、申诉；「待人工复核」是状态文案，不是审核后台。 |
| Worker 公平调度（`WORKER-DISPATCH-002`） | 仍可能按账号串行抽干；文案必须写清「不是高峰容量保证」。 |
| 完整 transcript / 删除回执 | `INT-TRANSCRIPT-00/01` 未关闭；复核页不得假装能回放逐题内容。 |
| 企业租户 / 席位 / ATS | 仍是个人招聘方账号骨架。 |
| 浏览器全链路发布证据 | 既有 `recruiting-bound` 规格可加复核入口断言；本切片不把本地静态门禁标成已发布。 |
...
3. 页脚写明：不是已部署的企业招聘产品；不构成能力认证；不能自动筛人、排名或决定录用。
...
- 说明页含七项人话标题，且同时出现「不提供自动筛选、排名、拒绝或录用决定」与「不构成能力认证」。
...
## UC-recruiter-review-01 · 打开岗位申请复核（只读最小投影）

**七类**：正✔ 异✔ 特✔ 逃✔ 并✔ 复✔ 刁✔

- **角色 Actor**：招聘方
- **前置 Precondition**：申请属于该招聘方自有岗位；列表 API 已在查询边界把 `score` 置 `NULL`。
- **触发 Trigger**：岗位候选人表或人才库点击「查看复核」。
...
- A1 人才库跨岗位跳进复核：URL 带该行 `job_id` + `application.id`。
- A2 `assessment_unavailable`：文案「待人工复核」，不是 0 分，也不是已完成录用。

**异常流 Exception**

| flow | 场景 | 机制 | 后置 |
| --- | --- | --- | --- |
| E1 重复打开 | 刷新复核页 | 只读 GET | 申请状态/分数不变 |
| E2 并发 | 候选人正在面试、招聘方同时打开复核 | 读投影；写仍只在候选人/worker CAS 路径 | 招聘方不能抢写分数 |
| E3 越权 | 改 URL 看他人申请，或把 C 端 `interviewId` 拼进复核 | RLS 列表为空 → `notFound`；URL 非法 id 直接 `notFound` | 不渲染他人行，不读 transcript |
...
| E6 断线 | 打开复核后断网 | 重开同一 URL 再读服务端列表 | 不靠客户端缓存当真相 |
...
| Proof-recruiter-review-逃 | 静态 UI | 复核页含「待人工复核」与「不提供数值评分」；无「综合评分」 |
| Proof-recruiter-review-并复 | 浏览器规格（既有 recruiting-bound） | 回填后招聘方不见「综合评分\|我的回答」；可见复核入口 |
