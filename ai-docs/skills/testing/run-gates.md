...
| `pnpm interview-dispatch:prove` | 远程 Postgres 公平调度负例（TC-WORKER-002）；缺配置失败关闭，不得改起本地 Docker | 集群 inflight / 发布 / per-push CI 绿。须 `E2E_CLOUD_ISOLATED=1`，**不**走 `run-e2e-isolated`，**不进** always-on。无库门是 `pnpm interview-dispatch:unit:prove` + `pnpm interview-dispatch:gate:prove` |
| `pnpm quiz:prove` / `pnpm reaper:prove` | 押题接线 / 孤儿收割在隔离 PG（#97 四门 130 迁移回执钉在 `33867570523` / `06b46c4`）。per-push `verify` 现跑这两门 | 双连接恰一领（`quiz-dual-claim:*`）、抽干顺序、支付渠道、发布证据。**不进** always-on |
