...
默认包含：`docs:check`、`golden-tasks:check`、`e2e-platform:check`、`e2e-platform:prove`、`e2e-helpers:prove`、`e2e-receipt:prove`、`e2e-runner:prove`、`arch`、`api:smoke`。目录契约与「AI 产出默认不可信」见 [E2E 目录契约](../../testing/conventions/e2e-directory-contract.md)。变更轮次用 `pnpm e2e-platform:loop`（可加 `--ui` / `--regression`）；不要把 loop 再嵌进 always-on。缺 Docker 不能假装隔离 prove 已跑。
