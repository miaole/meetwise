...
| --- | --- | --- | --- |
| `e2e/`、`scripts/run-e2e*.mjs`、`scripts/local-e2e-receipt*`、`scripts/e2e-static-guards*`、`scripts/e2e-fake-service-flags.mjs` | `pnpm e2e-helpers:prove` `pnpm e2e-receipt:prove` `pnpm e2e-runner:prove` `pnpm e2e-static-guards:check` `pnpm e2e-static-guards:prove` | `pnpm e2e:isolated` | 只改注释却声称全链路重跑 |
