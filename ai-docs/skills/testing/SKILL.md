...
| 明确不做 | 不在无 Key 的 CI 里假绿 live E2E；不把 Playwright 写成 HTTP 全链路的唯一实现；不把 planned golden-task 标成已通过；不把 `relatedCommands` 或 `scoring:eval` 写成 covering |
| 领域对象 | 无业务对象变更 |
| 状态机 | 无 |
| 契约 | 无 |
| 数据库 | 无 schema 变更；隔离 E2E 仍走完整迁移 |
| 测试计划 | `pnpm e2e-helpers:prove`、`pnpm e2e-receipt:prove`、`pnpm golden-tasks:check`、`pnpm golden-tasks:prove`、`pnpm docs:check`、`pnpm regression` |
...
- golden-tasks 第一批已建档；未映射到可跑门的条目状态必须是 `planned` 或 `unmapped`，禁止标 `passed`。附近 prove（`relatedCommands`）不是 covering；`scoring:eval` 不得写入 `mappedCommands`。
