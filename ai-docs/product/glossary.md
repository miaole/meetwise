...
| 已出题数 | InterviewView.issued_turns | 单场进度：privacy-active 面试上 `status<>'cancelled'` 的题目账本行数（含 issued/queued/answered） |
| 已答题数 | Overview.answered | 成长主页统计：privacy-active 面试上 `status='answered'` 的题目账本行数；单场对应 `answered_turns` |
