...
export function invokeHasNoDirectLeaseAcquire(src: string): boolean {
  const code = src.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');
  return !/\bacquireModelAdmission\b/.test(code)
    && !/\bai_model_admission_acquire(_scoped)?\b/.test(code)
    && !/\bai_model_concurrency_lease\b/.test(code);
}

export function partitionResolverFailsClosedWithoutOperation(src: string): boolean {
  return /if\s*\(\s*!spec\.operation\s*\)\s*return undefined;/.test(src)
    && /if\s*\(\s*!resolved\.ok\s*\)\s*return undefined;/.test(src);
}

A('无 operation → 分区 undefined（不进入 0120）', resolveModelAdmissionPartition({}) === undefined);
A('空 operation 字段不是分区',
  resolveModelAdmissionPartition({ operation: { id: '', businessRevision: '' } }) === undefined);
A('registry 未接线 operation 不派生分区',
  resolveModelAdmissionPartition({ operation: { id: 'qbank.embedding-build.v1', businessRevision: 'hc-gap-009' } }) === undefined);
...
A('invoke 不得直连 acquire / 0120 过程 / lease 表', invokeHasNoDirectLeaseAcquire(invokeSrc));
A('resolveModelAdmissionPartition 无 operation 或 resolve 失败都 return undefined',
...
const missingResolveGuard = admissionSrc.replace('if (!resolved.ok) return undefined;', 'if (!resolved.ok) { /* leak */ }');
A('静态负例：去掉 resolve 失败早退则失败', partitionResolverFailsClosedWithoutOperation(missingResolveGuard) === false);
const leakedAcquire = `${invokeSrc}\n  await acquireModelAdmission(c, input);\n`;
A('静态负例：invoke 直连 acquireModelAdmission 失败', invokeHasNoDirectLeaseAcquire(leakedAcquire) === false);
