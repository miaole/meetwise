...
async function main() {
  ...
  try {
    ...
    const unboundProductionClient = openAICompatibleClient({
      baseUrl: 'https://model.invalid', apiKey: 'test-only', model: 'unbound-production-proof',
    });
    ...
    A('context plan records an explicit bounded component total', planned.ok
      && planned.plan.inputTokens === planned.plan.systemTokens + planned.plan.userDataTokens
