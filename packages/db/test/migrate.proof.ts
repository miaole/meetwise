...
export async function runMigrateProof(
  ...
  A('0036 → 文本调用双向 token 价格与预留过程已建',
    ...
    && (await pool.query("SELECT 1 FROM pg_proc WHERE proname='ai_cost_reserve_text_scoped'")).rowCount === 1);
  ...
  A('0085/0088 → 逻辑节点 header/slot 与调用状态机由受控过程维护，app_role 没有 invocation 直写权限',
    ...
    && (await pool.query("SELECT has_table_privilege('app_role','ai_model_invocation','DELETE') allowed")).rows[0]?.allowed === false
    && (await pool.query("SELECT has_function_privilege('app_role','ai_model_claim_invocation_scoped(text,text,text,text,text,text,text,uuid,integer)','EXECUTE') allowed")).rows[0]?.allowed === true
