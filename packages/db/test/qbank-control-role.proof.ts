...
import {
  ...
  QBANK_CONTROL_DEFINER_FUNCTION_MANIFEST, QBANK_CONTROL_DEFINER_ROLE, QBANK_CONTROL_DEFINER_TABLE_MANIFEST, QBANK_CONTROL_DEFINER_VIEW_MANIFEST,
} from '../src/index.ts';
...
async function main() {
  ...
  A('QBank definer manifest includes taxonomy integrity helpers, §6.1-6.5 closure, and protected relations',
    ...
    && QBANK_CONTROL_DEFINER_FUNCTION_MANIFEST.some((entry) => entry.signature === 'qbank_generation_ann_search(text,vector,integer)')
    && QBANK_CONTROL_DEFINER_FUNCTION_MANIFEST.some((entry) => entry.signature === 'qbank_search_terms(text)'));
  ...
  try {
    ...
    // Migration 0073 revokes the migration role's PUBLIC function default, so a
    // freshly created function under the migration owner is no longer PUBLIC
    // EXECUTE.  The worst case is a rogue SECURITY DEFINER function that still
    // ends up PUBLIC callable (e.g. an operator grants it, or a pre-0073
    // creator default survives a transfer).  Grant PUBLIC EXECUTE explicitly to
    // prove the startup gate rejects it regardless of its visible ACL.
    await admin.query(
    ...
    await admin.query('GRANT EXECUTE ON FUNCTION qbank_rogue_owner_transfer_probe() TO PUBLIC');
