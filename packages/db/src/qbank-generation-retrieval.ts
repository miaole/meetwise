...
/** `undefined` means a pre-generation isolated test/database, where legacy qbank compatibility still applies. */
export async function activeQbankGeneration(c: Client): Promise<QbankActiveGeneration | undefined> {
...
export async function qbankQuestionResultsForHits(
  ...
  const questions = await qbankQuestionEvidenceForRefs(c, expectedRecipeId, hits.map((hit) => hit.refId), maxCharsPerPart);
...
export async function hybridQbankSearch(
  c: Client,
  input: { query: string; embedding: number[]; k: number; expectedRecipeId?: string; candidateK?: number; retrievalMode?: QbankRetrievalMode },
