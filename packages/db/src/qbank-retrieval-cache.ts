...
import { hybridQbankSearch, type QbankRetrievalMode, type QbankServingScopeInput } from './qbank-generation-retrieval.ts';

// v4 binds the track-local serving scope into the cache identity so two leaves
// never share a hot-cache entry (a global-Top-K hit must not leak across tracks).
const CACHE_KEY_VERSION = 'qbank-retrieval-cache:v4';
...
export interface QbankRetrievalCacheKeyInput {
  ...
  /** Track-local serving scope (RAG-FUNNEL-04): must be bound into the key so
   *  leaves never share an entry.  `taxonomyVersion`/`servingScopeId` pair. */
...
export interface CachedQbankSearchInput {
  ...
  qbankRecipeId?: string;
  retrievalMode?: QbankRetrievalMode;
  /** Track-local serving scope (RAG-FUNNEL-04).  Bound into the key and passed
   *  to the bounded retrieval readers so the DB predicate filters pre-ORDER BY. */
  scope?: QbankServingScopeInput;
...
export function qbankRetrievalCacheKey(input: QbankRetrievalCacheKeyInput): string {
  ...
  return createHmac('sha256', secret).update(JSON.stringify({
    ...
    k: input.k,
    query: input.query,
    servingScopeId: input.scope?.servingScopeId ?? null,
    taxonomyVersion: input.scope?.taxonomyVersion ?? null,
  })).digest('hex');
}
...
async function computeAnnAtEpoch(pool: DbPool, id: CacheIdentity, expectedEpoch: string, embedding: number[]): Promise<QbankRetrievalHit[] | undefined> {
  return asPrincipal(pool, id.owner, async (c) => {
    ...
    return hybridQbankSearch(c, {
      query: id.query, embedding, k: id.k, expectedRecipeId: id.qbankRecipeId, retrievalMode: id.retrievalMode, scope: id.scope,
    });
  });
}
...
export async function cachedQbankSearch(pool: DbPool, owner: string, input: CachedQbankSearchInput): Promise<CachedQbankSearchResult> {
  const key = qbankRetrievalCacheKey({ owner, query: input.query, k: input.k, embedderVersion: input.embedderVersion, qbankRecipeId: input.qbankRecipeId, retrievalMode: input.retrievalMode, scope: input.scope });
  const id: CacheIdentity = {
    owner, key, query: input.query, k: input.k, embedderVersion: input.embedderVersion,
    qbankRecipeId: input.qbankRecipeId, retrievalMode: input.retrievalMode ?? 'dense', scope: input.scope,
