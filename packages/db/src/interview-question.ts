...
export async function persistInterviewQuestion(
  ...
  const ins = await c.query(
    `INSERT INTO interview_question(owner_user_id,interview_id,question_id,state_version,turn,question,competency,qkind)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
       ON CONFLICT (owner_user_id,interview_id,question_id) DO NOTHING`,
    [owner, interviewId, q.questionId, q.stateVersion, q.turn, q.question, q.competency ?? null, q.qkind ?? null],
  );
  if (ins.rowCount === 1) return;
  const ex = await c.query(
    `SELECT state_version,turn,question,competency,qkind FROM interview_question
