-- 24_runtime_observability_gateway.sql — worker 运行时仅可读取的全局聚合指标。
-- 返回队列计数和单一费用 scope 摘要，不返回 job payload、用户、提示词、简历或逐笔账目。
...
BEGIN
  RETURN QUERY
    ...
    UNION ALL
    SELECT 'report'::text,
           count(*) FILTER (WHERE r.status = 'queued')::bigint,
           count(*) FILTER (WHERE r.status = 'running' AND r.lease_expires_at < clock_timestamp())::bigint,
           count(*) FILTER (WHERE r.status = 'quarantined')::bigint
    FROM public.ai_report AS r
    UNION ALL
    SELECT 'quiz_job'::text,
           count(*) FILTER (WHERE q.status = 'queued')::bigint,
           count(*) FILTER (WHERE q.status = 'running' AND q.lease_expires_at < clock_timestamp())::bigint,
           count(*) FILTER (WHERE q.status = 'failed')::bigint
    FROM public.quiz_job AS q
    UNION ALL
    SELECT 'diagnosis_job'::text,
           count(*) FILTER (WHERE d.status = 'queued')::bigint,
    ...
    FROM public.diagnosis_job AS d;
END;
$$;
...
BEGIN
  ...
  END IF;
  RETURN QUERY
    SELECT p.monthly_limit_micro_cny,
           coalesce(m.reserved_micro_cny, 0) + coalesce(m.settled_micro_cny, 0),
           (SELECT count(*)::bigint FROM public.ai_cost_reservation AS x
             WHERE x.scope_id = p.scope_id AND x.status = 'unknown')
    FROM public.ai_cost_budget_policy AS p
    LEFT JOIN public.ai_cost_budget_month AS m
      ON m.scope_id = p.scope_id
