...
-- permit is consumed by a BEFORE trigger as a second line of defence if a
-- future ACL drift accidentally restores a table write grant.
...
BEGIN
  ...
  IF OLD.status='claimed' AND NEW.status='failed' THEN
    IF NEW.lease_token IS NOT NULL OR NEW.lease_expires_at IS NOT NULL
      ...
      OR NEW.dispatched_at IS NOT DISTINCT FROM OLD.dispatched_at
      OR NEW.input_tokens IS DISTINCT FROM OLD.input_tokens
      OR NEW.output_tokens IS DISTINCT FROM OLD.output_tokens
      OR NEW.latency_ms IS DISTINCT FROM OLD.latency_ms THEN
      RAISE EXCEPTION 'ai_model_invocation_pre_dispatch_failure_invalid' USING ERRCODE='integrity_constraint_violation';
    END IF;
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  LOOP
    ...
    UPDATE ai_model_invocation
     ...
     WHERE owner_user_id=p_owner_user_id AND idempotency_key=candidate.idempotency_key
       AND status='dispatching';
    IF NOT FOUND THEN
...
REVOKE ALL ON FUNCTION ai_model_register_logical_node_header_scoped(text,text,text,text,text,text) FROM PUBLIC, app_role;
...
REVOKE ALL ON FUNCTION ai_model_claim_invocation_scoped(text,text,text,text,text,text,text,uuid,integer) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION ai_model_claim_invocation_scoped(text,text,text,text,text,text,text,uuid,integer) TO app_role;
