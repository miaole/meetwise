-- 0007_resume_quiz.sql — 押题流程(resume-quiz 图)增量迁移:押题结果表 + 生成 job 队列。
-- **非破坏**:CREATE TABLE/INDEX IF NOT EXISTS + 策略幂等守卫(重跑/已存在不报错,绝不 DROP 丢数据)。
-- 镜像 interview 形状:显式 status enum、owner FORCE RLS、version 乐观锁。sql/20_resume_quiz.sql 为同形单文件镜像(供 proof 直跑)。

CREATE TABLE IF NOT EXISTS resume_quiz (
  id text PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS ix_quiz_owner ON resume_quiz (owner_user_id, status);
...
CREATE TABLE IF NOT EXISTS quiz_job (
  ...
  version int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT uq_quiz_job UNIQUE (owner_user_id, quiz_id)
);
CREATE INDEX IF NOT EXISTS ix_quizjob_claim ON quiz_job (owner_user_id, status, created_at);
