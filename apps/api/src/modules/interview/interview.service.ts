...
import { assertInterviewPrivacyActive, reserveEntitlement, enqueueInterviewJob, getReport, abandonInterviewAndRelease, requeueFailedReport, claimInterviewAnswer } from '@meetwise/db';
...
/** A numeric event is not a score source unless the worker bound it to the
 * server-issued question and answer identity.  This excludes legacy `/answer`
 * rows without making a best-effort guess from their numeric score. */
function isProvenanceQualifiedScore(payload: any): boolean {
  const score = typeof payload?.score === 'number'
    ? payload.score
    : (typeof payload?.score === 'string' && /^[0-9]+(?:\.[0-9]+)?$/.test(payload.score) ? Number(payload.score) : Number.NaN);
...
export class InterviewService {
  ...
  transcript(principal: string, id: string) {
    return this.db.asPrincipal(principal, async (c) => {
      ...
      return { turns: ev.rows.filter((r: any) => isProvenanceQualifiedScore(r.payload)).map((r: any, i: number) => {
        const p = r.payload ?? {};
  ...
  generateAssessment(principal: string, id: string) {
    return this.db.asPrincipal(principal, async (c) => {
      ...
      const turns = ev.rows
        .map((r: any) => r.payload)
        .filter((p: any) => isProvenanceQualifiedScore(p))
