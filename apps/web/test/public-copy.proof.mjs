...
const checks = {
  ...
  'TC-PUBLIC-COPY-E9': () => {
    ...
    requireText(recruiterTalent, '内部预览', 'recruiter talent page');
    requireText(recruiterTalent, '不是人才库产品', 'recruiter talent page');
    requireText(recruiterTalent, '也不是招聘方产品', 'recruiter talent page');
    forbid(features, ['CRAG', '自动触发再检索', '自主探索'], 'feature claims');
    requireText(recruiterHow, '怎么评估', 'recruiter architecture page');
    requireText(recruiterHow, '架构笔记', 'recruiter architecture page');
    requireText(recruiterHow, '招聘方产品没有上线', 'recruiter architecture page');
    requireText(recruiterHighlights, '内部架构笔记', 'recruiter architecture highlights');
    requireText(recruiterPreviewNote, '内部笔记', 'recruiter preview note');
    requireText(recruiterPreviewNote, '不是「求职者 / 面试官」两套对等产品', 'recruiter preview note');
    requireText(recruiterPreviewNote, '招聘方产品没有上线', 'recruiter preview note');
    ...
    forbid(recruiterHighlights, ['面试官能指望什么', '招聘方产品已上线'], 'recruiter architecture highlights');
    ...
    forbid(recruiterHow + recruiterHighlights + recruiterSurface + recruiterPreviewNote, ['Grok', '生产级可靠', '自动录用已开放', '排队按申请分开领', '检索只在授权范围内', '招聘方产品已上线'], 'recruiter architecture copy');
    ...
    requireText(recruiterJobs, '内部预览', 'recruiter jobs page');
