'use client';
/**
 * 全程语音「电话模式」:考官提问用 TTS 朗读,候选人用语音作答,转写后提交,等下一题继续——像打电话一样连续。
 ...
 *  - 报告就绪 / 报告不可用 / 面试不可用 → 朗读收尾或给出路,结束通话,绝不空转。
 */
...
export function VoiceCallPanel({
  ...
  const processedIdxRef = useRef<number>(-1);          // 已处理(已朗读+录音)的题序号,防重复朗读
  ...
  const speak = useCallback(async (text: string, gen: number): Promise<void> => {
    setStatus('speaking');
    setTtsPhase('synth');
    // **可靠性优先**:流式 MSE 播放(cosyvoice MP3)未经真机验证、且"起播≠出声"会让通话静默卡死,
    ...
    try {
      ...
      const url = URL.createObjectURL(base64ToBlob(audioBase64, mimeType || 'audio/wav'));
  ...
  const recordAnswer = useCallback(async (gen: number): Promise<Blob | null> => {
    ...
    let speech = false, silenceStart = 0, hintShown = false;
    ...
    const tick = () => {
      ...
      if (rms > SPEECH_ON) { speech = true; silenceStart = 0; }
      else if (speech) {
  ...
  /** 一道题的完整回合:朗读 → 录音 → 转写 → 提交。idx 即作答轮次(turn)。 */
  const runTurn = useCallback(async (idx: number, questionText: string) => {
    const gen = genRef.current;
    await speak(questionText, gen);
    if (gen !== genRef.current) return;
    ...
    if (!text.trim()) {                                  // 没识别到 → 再听一次,不提交空答
      setHint('没听清,请再说一遍');
      void runTurnRetryRef.current?.(idx, questionText);
      return;
    }
    ...
    try {
      const res = await fetch(`/api/interview/${encodeURIComponent(resultId)}/turn`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'idempotency-key': `${resultId}:turn:${idx}` },
        body: JSON.stringify({ turn: idx, answer: text }),
      });
      // HTTP 4xx/5xx 不是 throw:必须显式判 res.ok,否则会落到 thinking 永久空转(无 SSE 推进、电话模式无 30s 兜底)。
      if (!res.ok) { if (gen === genRef.current) { toast.error('回答提交失败,请「再试一次语音」或切回打字'); setStatus('asr_down'); } return; }
    } catch { if (gen === genRef.current) { toast.error('回答提交失败,请「再试一次语音」或切回打字'); setStatus('asr_down'); } return; }
    if (gen !== genRef.current) return;
    setStatus('thinking');                               // 等下一题 question_ready / report_ready
  }, [recordAnswer, resultId, speak]);

  // 「再听一次」复用录音→转写→提交那段(不重读题),用 ref 解循环依赖
  const runTurnRetryRef = useRef<((idx: number, q: string) => Promise<void>) | null>(null);
  runTurnRetryRef.current = async (idx, q) => {
    const gen = genRef.current;
    const blob = await recordAnswer(gen);
    ...
    try {
      ...
      const sres = await fetch(`/api/interview/${encodeURIComponent(resultId)}/turn`, {
        method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': `${resultId}:turn:${idx}` },
        body: JSON.stringify({ turn: idx, answer: text }),
      });
      if (!sres.ok) { if (gen === genRef.current) { toast.error('回答提交失败,请「再试一次语音」或切回打字'); setStatus('asr_down'); } return; }
      if (gen === genRef.current) setStatus('thinking');
    } catch { if (gen === genRef.current) { setStatus('asr_down'); toast.error('语音转写失败,可切回打字继续'); } }
  };

  // 主驱动:监听 view/display 变化,推进语音回合
  useEffect(() => {
    if (status === 'ended' || status === 'mic_denied' || status === 'asr_down') return; // 等用户出路,别自动重启

    // 终态降级(报告不可用 / 面试不可用 / error)→ 给出路,结束通话
    ...
    if (display.report && !reportSpokenRef.current) {
      ...
      setStatus('report');
      void speak(`本次模拟面试结束,综合评分 ${display.report.overall} 分。感谢你的参与,可在屏幕上查看完整报告。`, genRef.current);
      return;
    }

    // 新题 → 开启一个回合(processedIdxRef 防重复朗读)
    if (canAnswer && lastIdx >= 0 && processedIdxRef.current !== lastIdx) {
      processedIdxRef.current = lastIdx;
      void runTurn(lastIdx, view.turns[lastIdx].q);
    }
    // 评估中/出题中(已作答未出新题)→ thinking
    else if (!canAnswer && !display.report && !display.degraded && processedIdxRef.current === lastIdx && status !== 'speaking' && status !== 'listening') {
      setStatus((s) => (s === 'submitting' || s === 'transcribing' ? s : 'thinking'));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view.turns.length, display.action.kind, display.report?.overall, display.degraded]);
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-lg flex-col">
      ...
      <header className="flex items-center justify-between border-b pb-3">
        <h2 className="font-serif text-lg">📞 电话面试</h2>
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-6 py-8 text-center">
        ...
        <div className="relative grid place-items-center">
          ...
          <div className={`relative grid size-28 place-items-center rounded-full border-2 transition-colors ${
            micActive ? 'border-primary bg-accent'
              ...
              : (status === 'mic_denied' || status === 'asr_down' || status === 'degraded' || status === 'ended') ? 'border-destructive/40 bg-card'
            ...
            {status === 'speaking' && ttsPhase === 'playing' ? <Volume2 className="size-9 text-primary motion-safe:animate-pulse" />
              ...
              : (status === 'mic_denied' || status === 'asr_down' || status === 'degraded' || status === 'ended') ? <AlertTriangle className="size-9 text-destructive" />
        ...
        {status === 'report' && display.report && (
          <div className="w-full rounded-lg border bg-accent p-5 text-center text-accent-foreground">
            面试完成 · 综合评分 <span className="text-3xl font-extrabold text-primary">{display.report.overall}</span>
            <div className="mt-2 text-xs text-muted-foreground">通话已结束,可切回查看完整报告</div>
          </div>
        )}
      ...
      <div className="space-y-3 border-t pt-4">
        ...
        {status === 'asr_down' && (
          <button
            type="button"
            onClick={() => { setHint(null); processedIdxRef.current = lastIdx; if (lastIdx >= 0) void runTurn(lastIdx, view.turns[lastIdx]?.q ?? currentQ); }}  // 直接重开本题回合(主驱动 effect 不依赖 status,不能靠它重启)
        ...
        <div className="flex items-center gap-3">
          ...
          {status === 'ended' ? (
            ...
            >
              <PhoneOff className="size-4" />挂断
            </button>
          )}
        </div>
        {status === 'ended' && <p className="text-center text-xs text-muted-foreground">已挂断语音通话。进度已保存,可返回继续或切回打字。</p>}
      </div>
    </div>
  );
}
...
function WaveBars({ wave }: { wave: number[] }) {
  return (
    <div className="flex h-full w-full items-center justify-center gap-[3px]" role="img" aria-label="实时语音波形">
      {wave.map((v, i) => (
        <span
          key={i}
