...
type Strategy = 'dense' | 'bm25' | 'rrf_top10' | 'rrf_candidate_pool_shaped' | 'rrf_dense_1_bm25_0_5' | 'rerank_rrf_candidate_pool_shaped';
...
async function main() {
  ...
  const rankings: Record<Strategy, string[][]> = {
    ...
    rrf_candidate_pool_shaped: denseCandidates.map((ids, i) => rrf([ids, lexicalCandidates[i]], K)),
    ...
    rerank_rrf_candidate_pool_shaped: [],
  };
  ...
  const printAnswerableReport = (title: string, qs: typeof answerable) => {
    ...
    for (const mode of ['dense', 'bm25', 'rrf_top10', 'rrf_candidate_pool_shaped', 'rrf_dense_1_bm25_0_5'] as const) {
  ...
  if (ENABLE_RERANK) {
    ...
    for (const q of answerable) {
      ...
      rankings.rerank_rrf_candidate_pool_shaped[qi] = ordered;
      if ((latencies.length % 10) === 0) console.log(`  rerank ${latencies.length}/${answerable.length}`);
    }
    const r = report('rerank_rrf_candidate_pool_shaped');
    console.log(`rerank_rrf_candidate_pool_shaped hit@${K}=${pct(r.hitRate)} recall@${K}=${pct(r.recall)} strict-all=${pct(r.successRate)} (${r.fullHits}/${r.n}, Wilson95%下界=${pct(r.lcb)}) MRR=${r.mrr.toFixed(3)} nDCG=${r.ndcg.toFixed(3)} MAP=${r.map.toFixed(3)} rerank_latency_ms(p50/p95)=${percentile(latencies, 0.5).toFixed(1)}/${percentile(latencies, 0.95).toFixed(1)}`);
  } else {
