...
import { annSearch, asPrincipal, createPool, upsertVectorChunk } from '@meetwise/db';
...
async function main() {
  ...
  try {
    ...
    for (const vec of qVecs) {
      const rows = await asPrincipal(pool, reader, (c) => annSearch(c, reader, 'qbank', vec, K));
    ...
    console.log('✓ 已覆盖 legacy compatibility 路径：真实 embedding → pgvector（HNSW 已建）→ app_role/RLS → qbank_visible_ref → annSearch。它不覆盖 generation-aware hybridQbankSearch、artifact evidence 或 serving cache。上方 plan 只代表 24-chunk fixture；不能外推为 10 万+ 语料性能。临时数据库将删除。');
  } finally {
    await pool.end();
