...
type QbankFact = { refId: string; contentHash: string; content: string };
...
async function snapshotFacts(pool: DbPool): Promise<{ epoch: string; facts: QbankFact[] }> {
  return asQbankControlExecutor(pool, async (c) => {
    ...
    return {
      ...
      facts: facts.rows.map((r) => ({ refId: String(r.ref_id), contentHash: String(r.content_hash), content: String(r.content) })),
    };
  });
}

async function insertGenerationRows(pool: DbPool, generationId: string, facts: QbankFact[], vectors: number[][]): Promise<void> {
  for (let start = 0; start < facts.length; start += INSERT_BATCH) {
    ...
    await asQbankControlExecutor(pool, (c) => c.query(
      `INSERT INTO qbank_generation_chunk(generation_id,ref_id,content_hash,embedding) VALUES ${values}
       ON CONFLICT (generation_id,ref_id) DO NOTHING`, params,
    ));
  }
}
...
export async function ensureActiveQbankGeneration(pool: DbPool, embedder: Embedder): Promise<QbankGenerationResult | undefined> {
  ...
  const snapshot = await snapshotFacts(pool);
  ...
  try {
    ...
    await insertGenerationRows(pool, generationId, snapshot.facts, vectors);
