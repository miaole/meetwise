 ...
 */
import { asPrincipal, claimNextDiagnosisJob, markDiagnosisJobDone, markDiagnosisJobFailed, appendEvent, releaseConsumption, renewDiagnosisJobLease, sweepStuckDiagnosisJobs, type DbPool } from '@meetwise/db';
...
export async function drainDiagnosisJobOnce(d: DiagnosisConsumerDeps, owner: string): Promise<DiagnosisDrainResult> {
  ...
  try {
    await runDiagnosis(d.pool, owner, job.diagnosisId, job.payload.resumeId, job.payload.role, d.model);
    ...
    await asPrincipal(d.pool, owner, async (c) => {
      ...
      const stillMine = await markDiagnosisJobFailed(c, owner, job.id, d.leaseOwner, e?.message ?? 'err');
      if (!stillMine) return;
      ...
      await appendEvent(c, owner, job.diagnosisId, 'diagnosis_unavailable', { reason: 'job_failed' });
...
/** 一拍调度:枚举有待办诊断 job 的 owner(需越 RLS)→ 每 owner 先 reap(收割孤儿) 再 drain。 */
export async function diagnosisDispatchTick(d: DiagnosisConsumerDeps): Promise<{ owners: number; requeued: number; failed: number }> {
  const owners = (await d.pool.query("SELECT DISTINCT owner_user_id FROM diagnosis_job WHERE status='queued' OR (status='running' AND lease_expires_at < now())")).rows.map((r) => r.owner_user_id as string);
...
export function runDiagnosisConsumer(d: DiagnosisConsumerDeps, intervalMs = 1500) {
