...
import { invoke, getPrompt, promptedModel, openAICompatibleClient, circuitBreaker, rateLimitedModel, failoverModel, type ModelClient, type ModelCostPolicy } from '@meetwise/ai-runtime';
...
export async function invokeEvaluationOnce(pool: DbPool, owner: string, input: EvaluationInvokeInput): Promise<EvaluationInvokeResult> {
  ...
  const primary = await invoke({
    ...
    logicalNodeKey: primaryIdempotencyKey,
...
function endpoint(cfg: { baseUrl?: string; apiKey?: string; model?: string; costPolicy?: ModelCostPolicy } = {}): ModelClient {
  return circuitBreaker(rateLimitedModel(openAICompatibleClient(cfg)));
}
/** 跨供应商 failover（故障转移）链：primary 由 MODEL_* 指定；配了 MODEL_BACKUP_BASE_URL 才启用 backup。
 *  仅 primary 在派发前已知不可用（例如熔断打开）时才选 backup。429/超时等已派发结果必须冻结为 unknown，
 *  不能把“不确定是否已扣费”误做秒级重发。不配 backup 则等价单端点（向后兼容）。 */
function withFailover(model?: string, costs: { primary?: ModelCostPolicy; backup?: ModelCostPolicy } = {}, backupModel?: string): ModelClient {
  ...
  const backup = endpoint({ baseUrl: process.env.MODEL_BACKUP_BASE_URL, apiKey: process.env.MODEL_BACKUP_API_KEY, model: backupModel ?? process.env.MODEL_BACKUP_NAME ?? model, costPolicy: costs.backup });
...
export function quizGenerator(pool: DbPool, owner: string, resumeFacts: string[], idempotencyKey: string, model: ModelClient): GenerateQuestions {
  return async (): Promise<QuizItem[]> => {
    const out = await invoke({
      idempotencyKey, logicalNodeKey: idempotencyKey, schema: QuizSchema,
...
export function diagnosisGenerator(pool: DbPool, owner: string, resumeFacts: string[], role: string | undefined, idempotencyKey: string, model: ModelClient): GenerateDiagnosis {
  return async (): Promise<RawDiagnosis> => {
    const out = await invoke({
      idempotencyKey, logicalNodeKey: idempotencyKey, schema: DiagnosisSchema, service: 'resume-diagnosis.generate',
      businessValidate: (v) => {
...
export function reportGenerator(pool: DbPool, owner: string, idempotencyKey: string, model: ModelClient): GenerateReport {
  return async (s: InterviewSummary): Promise<ReportContent> => {
    ...
    const out = await invoke({
      idempotencyKey, logicalNodeKey: idempotencyKey, schema: ReportSchema,
      businessValidate: (v) => {
