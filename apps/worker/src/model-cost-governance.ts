...
import { assertTextEndpointKeyFingerprints, isTextBackupEnabled, resolveTextBackupEndpointConfig, resolveTextEndpointConfig } from '@meetwise/ai-runtime';
...
export function resolveModelCostGovernance(env: NodeJS.ProcessEnv = process.env): ModelCostGovernanceConfig {
  ...
  // BAILIAN-04 endpoint 治理:文本 endpoint 只能来自版本化 profile 注册表(精确 https host/path)。
  // 旧的自由 URL(MODEL_BASE_URL/MODEL_BACKUP_BASE_URL)出现即抛 text_endpoint_env_forbidden——
  // 不在这里拒绝,旧变量就会静默残留成 SSRF/明文/跨区/跟随重定向的入口。放在计费解析之前,
  // 保证「没通过 endpoint 治理的配置」根本走不到价格/预算,启动即 fail-closed。
  resolveTextEndpointConfig(env);
  // BAILIAN-03 轮换残留机械校验:挂载的 Key 命中撤销指纹清单或与期望指纹不一致 → 拒绝启动。
  // 为什么机械化:靠人眼比对新 Key 和期望指纹不可靠,轮换后旧 Key 残留必须由启动断言堵死。
  assertTextEndpointKeyFingerprints(env);
  ...
  // 备用 endpoint 的启用开关是「备用 Key 已挂载」(isTextBackupEnabled = MODEL_BACKUP_API_KEY 非空),
  // 不再是旧的自由 URL。为什么:URL 本身既不可信也不能证明 Key 存在;以 Key 为开关才能
  // 保证「选了 backup 就必有可用的备用凭据」,并让旧 MODECEL_BACKUP_BASE_URL 彻底退出语义。
  if (isTextBackupEnabled(env)) {
    // 备用 endpoint 同样过受控 profile 解析(含 legacy URL 拒绝),不能靠自由 URL 放行。
    resolveTextBackupEndpointConfig(env);
    // 备用 Key 是 opaque secret:只校验存在与可打印形状,绝不日志/规范化其值。
