...
export async function planCompetencies(pool: DbPool, owner: string, threadId: string, model: ModelClient, role: string, facts: string[]): Promise<CompetencySpec[]> {
  const out = await invoke({
    idempotencyKey: `${threadId}:plan`, logicalNodeKey: `${threadId}:plan`, threadId, privacyInterviewId: threadId,
  ...
  return {
    ...
    async retrieveAndGenerate(competency, difficulty, attempt, turn, _facts, kind) {
      ...
      const generate = () => invoke({
          idempotencyKey: `${d.threadId}:ask:t${turn}:0`, logicalNodeKey: `${d.threadId}:ask:t${turn}`,
          ...
          model: promptedModel(d.model, 'interviewer.ask', { competency, difficulty, kind, material, resumeFacts: [] }),
        }, d.pool, d.owner);
