...
const items: QbankItem[] = [
  { refId: 'itest:a', text: '灌库治理证明题 A:谈谈幂等与去重的取舍' },
  { refId: 'itest:b', text: '灌库治理证明题 B:撤销源如何即时下架其真实 chunk' },
  { refId: 'itest:c', text: '灌库治理证明题 C:崩溃半态如何原子恢复' },
];
...
async function main() {
  ...
  console.log('\n──────── ③ 灌库幂等 + ④ 被撤销种子不复活 ────────');
  const n2 = await ingestQbank(pool, items, emb);           // 重灌:b 已 rejected 应被跳过
  ...
  const hd = hashOf(DUP);
  const nd = await ingestQbank(pool, [{ refId: 'itest:d1', text: DUP }, { refId: 'itest:d2', text: DUP }], emb);
  ...
  await ingestQbank(pool, [{ refId: 'itest:e', text: '改版守卫 v1' }], emb);
  let revErr = false;
  try { await ingestQbank(pool, [{ refId: 'itest:e', text: '改版守卫 v2(同 refId 换内容)' }], emb); }
  catch { revErr = true; }
