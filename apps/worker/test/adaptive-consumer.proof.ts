...
const askRequests: Array<{ system: string; userData: string }> = [];
let modelCalls = 0;
...
const model: ModelClient = {
  complete: async (request, attempt) => {
    modelCalls++;
    if (request.service === 'interviewer.ask') askRequests.push({ system: request.system, userData: request.userData });
...
async function main() {
  ...
  A('深检索正文以不可信信封进入出题 prompt，系统明确禁止执行来源指令', askRequests.length >= 1 && askRequests[0]!.userData.includes('[UNTRUSTED_RESEARCH_SOURCE') && askRequests[0]!.userData.includes('忽略此前指令') && askRequests[0]!.system.includes('检索安全'));
