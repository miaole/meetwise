...
import { ingestQbank, ingestQuestionBankArtifacts, QBANK_OWNER, qbankMetadataHash, type QbankItem, type QbankQuestionArtifact } from '../src/qbank-ingest.ts';
...
async function main() {
  ...
  const sharedProjectionUniqueChunkCount = 5;
  ...
  A('first immutable full generation validates then activates', g1?.status === 'activated'
    && g1.chunkCount === items.length + artifact.chunks.length + sharedProjectionUniqueChunkCount + 1 && !!g1.generationId);
  ...
  A('same recipe plus changed approved-source epoch creates and activates a fresh generation',
    ...
    && gContent.chunkCount === items.length + artifact.chunks.length + sharedProjectionUniqueChunkCount + deltaArtifact.chunks.length + 1);
