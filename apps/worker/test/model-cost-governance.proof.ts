...
const backup = resolveModelCostGovernance({
  ...base, MODEL_BACKUP_API_KEY: 'backup-test-only', MODEL_BACKUP_NAME: 'qwen-plus-backup', MODEL_FAST_BACKUP_NAME: 'qwen-turbo-backup',
  ...endpoint('MODEL_BACKUP', 'qwen-plus-backup'), ...endpoint('MODEL_FAST_BACKUP', 'qwen-turbo-backup'),
});
A('启用备用端点（以备用 Key 挂载为准）时，两个备用模型均须有独立费率且被加载', backup.priceRows.length === 4 && backup.policies.backup?.model === 'qwen-plus-backup' && backup.policies.fastBackup?.model === 'qwen-turbo-backup');
// BAILIAN-04: 备用 endpoint 的启用开关从「自由 URL 存在」改为「备用 Key 已挂载」。仅有 backup
// profile 而没有 backup Key 时，不启用备用（不 reject，只出 primary/fast 2 行）——URL 不再有开关语义。
const profileOnlyBackup = resolveModelCostGovernance({
  ...base, MODEL_BACKUP_ENDPOINT_PROFILE: 'dashscope-cn-beijing', MODEL_BACKUP_NAME: 'qwen-plus-backup', MODEL_FAST_BACKUP_NAME: 'qwen-turbo-backup',
  ...endpoint('MODEL_BACKUP', 'qwen-plus-backup'), ...endpoint('MODEL_FAST_BACKUP', 'qwen-turbo-backup'),
});
A('仅有 backup profile 没有 backup Key 时不启用备用端点（仅 primary/fast 2 行，不 reject）',
  profileOnlyBackup.priceRows.length === 2 && profileOnlyBackup.policies.backup === undefined);
A('旧自由 URL（MODEL_BASE_URL/MODEL_BACKUP_BASE_URL）出现即拒绝启动（endpoint 治理 fail-closed）',
  fails({ ...base, MODEL_BASE_URL: 'https://free.invalid' }, 'text_endpoint_env_forbidden')
  && fails({ ...base, MODEL_BACKUP_BASE_URL: 'https://free-backup.invalid', MODEL_BACKUP_API_KEY: 'k' }, 'text_endpoint_env_forbidden'));
