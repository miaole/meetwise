...
import { buildAdaptiveInterviewGraph, type AdaptiveDeps } from '@meetwise/ai-graphs';
...
async function main() {
  /* ───── (a) 非作答 → clarify:重发同一题、难度不变、depthProbed 不增、非更深题 ───── */
  {
    ...
    await g.invoke(new Command({ resume: '不知道' }), cfg);   // 确定性非作答(套话)
    ...
    await g.invoke(new Command({ resume: '还是不会' }), cfg);
    snap = await g.getState(cfg);
  ...
  /* ───── (c) 好答 → move on 下一能力(不过度追问) ───── */
  {
    ...
    await g.invoke(new Command({ resume: '我在项目里用读写分离+本地缓存抗住了峰值,权衡了一致性与延迟,最终选最终一致' }), cfg);
    snap = await g.getState(cfg);
  ...
  /* ───── (d) 长篇跑题(模型 relevant=false)→ 当非作答处置(非弱答加深) ───── */
  {
    ...
    await g.invoke(new Command({ resume: `让我聊聊我的家乡和我最喜欢的电影以及周末爱做的事情,反正跟题目无关 ${OFFTOPIC} 巴拉巴拉一大段` }), cfg);
    snap = await g.getState(cfg);
  ...
  /* ───── 跳过即换题(显式 skip 不澄清、不 penalty-loop)───── */
  {
    ...
    await g.invoke(new Command({ resume: '跳过' }), cfg);   // 显式跳过(首次即换题,不先澄清)
    snap = await g.getState(cfg);
  ...
  /* ───── (e) 整场必在预算内收尾:全程非作答也不死循环 ───── */
  {
    ...
    while (res.__interrupt__ && guard++ < 40) res = await g.invoke(new Command({ resume: '不知道' }), cfg);   // 每轮都非作答
