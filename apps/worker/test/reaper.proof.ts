...
const orphanInterview = (id: string, attempts: number) => pool.query(
  ...
  [id, attempts]);
const orphanQuiz = (attempts: number) => pool.query(
  `UPDATE quiz_job SET status='running', lease_owner='dead-worker#1', lease_expires_at=now()-interval '5 minutes', attempts=$2 WHERE quiz_id=$1`,
  [QID, attempts]);
const jobRow = (table: string, col: string, id: string) =>
  pool.query(`SELECT status, lease_owner, attempts FROM ${table} WHERE ${col}=$1`, [id]).then((r) => r.rows[0]);
...
async function main() {
  for (const f of ['01_schema', '02_commerce', '03_resume', '14_notification', '05_interview_jobs', '20_resume_quiz']) await pool.query(sql(`../../../packages/db/sql/${f}.sql`));
  await pool.query(`INSERT INTO interview(id,owner_user_id,status) VALUES ('${IID}','${OWNER}','active'),('${CID}','${OWNER}','active')`);
  await pool.query(`INSERT INTO resume_quiz(id,owner_user_id,status) VALUES ('${QID}','${OWNER}','generating')`);
  await pool.query(`INSERT INTO entitlement_bucket(owner_user_id,kind,units_total,expires_at) VALUES ('${OWNER}','paid',10.0, now()+interval '300 days')`);
  ...
  await pool.query(`INSERT INTO interview_job(owner_user_id,interview_id,kind,seq,payload) VALUES ('${OWNER}','${IID}','start',0,'{}')`);
  await pool.query(`INSERT INTO quiz_job(owner_user_id,quiz_id,payload) VALUES ('${OWNER}','${QID}','{}')`);
  ...
  await pool.query(`UPDATE interview_job SET status='running', lease_owner='reaper-1', lease_expires_at=now()-interval '5 minutes', attempts=1 WHERE interview_id='${IID}'`);
  ...
  section('⑤ 僵尸兄弟守卫:面试已有 failed job → 后续 seq job 不被 claim(不乱序跑已死面试)');
  // IID 的 start(seq0)已 failed(section③)。新入队 answer(seq1)——claim 必须拒领(否则对已宣告 unavailable/已退款的面试乱序跑题)。
  await pool.query(`INSERT INTO interview_job(owner_user_id,interview_id,kind,seq,payload) VALUES ('${OWNER}','${IID}','answer',1,'{}')`);
  const claimed = await asPrincipal(pool, OWNER, (c) => claimNextInterviewJob(c, OWNER, 'reaper-1'));
  A('有 failed 兄弟 → claim 返回 null(不领已死面试的后续题)', claimed === null);
  ...
  await pool.query(`INSERT INTO interview_job(owner_user_id,interview_id,kind,seq,payload) VALUES ('${OWNER}','${CID}','start',0,'{}')`);
...
async function jobId(table: string, col: string, id: string): Promise<string> {
  const r = await pool.query(`SELECT id FROM ${table} WHERE ${col}=$1`, [id]);
  return r.rows[0].id;
}
