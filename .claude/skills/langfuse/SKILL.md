...
```
`LANGFUSE_HOST` is the canonical server variable for the CLI; `LANGFUSE_BASE_URL` is an accepted alias (the SDK name for the same value). Set exactly one of them — they must not conflict.
