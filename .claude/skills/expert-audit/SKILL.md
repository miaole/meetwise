...
3. 用 Task/Agent 并行派专家（复杂面用 Workflow 多 agent + 综合）。每条发现给 `严重度(致命/高/中) | 问题 | 改法`。
4. **综合成 fix-list**，按严重度排序，先修致命。
5. **应用修复并重新验证**（如代码 gate 重跑），闭合"发现→修→验"循环。未闭合不算完成。

## 输出

`审计报告(分级 findings + 过度声明合订 + solid 保留项 + 修复顺序)` → 应用 → `验证通过`。
